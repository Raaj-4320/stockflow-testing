  ...
  if (effectiveTransaction.customerId) {
      ...
      if (effectiveTransaction.type === 'sale') {
          ...
          console.info('[FIN][RETURN][SETTLEMENT]', {
            ...
            customerId: c.id,
            total: amount,
            mode: returnEffects.mode,
            cashImpact: returnEffects.affectsCash ? -amount : 0,
            dueImpact: returnEffects.affectsDue ? -amount : 0,
            storeCreditImpact: returnEffects.mode === 'store_credit' ? amount : 0,
            onlineRefundImpact: returnEffects.mode === 'refund_online' ? -amount : 0,
          });
      ...
      console.info('[FIN][LEDGER][RESULT]', {
        ...
        finalDue: totalDue,
        finalStoreCredit: storeCredit,
      });
  }
  if (effectiveTransaction.type === 'sale') {
    const gross = effectiveTransaction.items.reduce((sum, item) => sum + (item.sellPrice * item.quantity), 0);
    const net = Math.abs(effectiveTransaction.total);
    financeLog.pnl('EVENT', { txId: effectiveTransaction.id, type: 'sale', gross, discount: Math.max(0, gross - net), tax: 0, net });
  } else if (effectiveTransaction.type === 'return') {
    const returnEffects = getReturnFinancialEffects(effectiveTransaction);
    financeLog.pnl('EVENT', { txId: effectiveTransaction.id, type: 'return', gross: 0, discount: 0, tax: 0, net: -txAmount, mode: returnEffects.mode });
  }
...
export const deleteTransaction = (transactionId: string): Transaction[] => {
  ...
  console.info('[FIN][RECONCILE][DELETE]', {
    ...
    beforeDue: deletedRecord.beforeImpact.customerDue,
    afterDue: deletedRecord.afterImpact.customerDue,
    beforeStoreCredit: deletedRecord.beforeImpact.customerStoreCredit,
    afterStoreCredit: deletedRecord.afterImpact.customerStoreCredit,
  });
  if (target.customerId) {
    console.info('[FIN][LEDGER][RESULT]', {
      ...
      reason: 'delete',
      finalDue: deletedRecord.afterImpact.customerDue,
      finalStoreCredit: deletedRecord.afterImpact.customerStoreCredit,
    });
  }
...
export const updateTransaction = async (updatedTransaction: Transaction): Promise<Transaction[]> => {
  ...
  affectedCustomerIds.forEach((customerId) => {
    ...
    console.info('[FIN][LEDGER][RESULT]', {
      ...
      customerId,
      reason: 'update',
      finalDue: toFiniteNonNegative(customer.totalDue),
      finalStoreCredit: toFiniteNonNegative(customer.storeCredit),
    });
  });
