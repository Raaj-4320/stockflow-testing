...
const commitProcessTransactionAtomically = async ({
  ...
  return runFirestoreTransaction(db!, async (firestoreTx) => {
    ...
    if (transaction.customerId) {
      ...
      const customerSnap = await firestoreTx.get(customerRef);
      ...
      if (transaction.type !== 'payment') {
        for (const item of transaction.items) {
          ...
          if (!statsSnap.exists() && !allowLegacySeed) {
...
const syncFromCloud = async () => {
    ...
    try {
        ...
        unsubscribeProductsSnapshot = onSnapshot(getProductsCollectionRef(user.uid), (productsSnap) => {
            ...
            // Compatibility guard: do not overwrite legacy fallback products with [] while subcollection is not populated yet.
        ...
        unsubscribeCustomersSnapshot = onSnapshot(getCustomersCollectionRef(user.uid), (customersSnap) => {
            ...
            // Compatibility guard: do not overwrite legacy fallback customers with [] while subcollection is not populated yet.
        ...
        unsubscribeTransactionsSnapshot = onSnapshot(getTransactionsCollectionRef(user.uid), (transactionsSnap) => {
            ...
            // Compatibility guard: do not overwrite legacy fallback transactions with [] while subcollection is not populated yet.
            if (transactions.length > 0) {
              memoryState = { ...memoryState, transactions };
              window.dispatchEvent(new Event('local-storage-update'));
            }
        }, (error) => {
        ...
        unsubscribeSnapshot = onSnapshot(docRef, async (docSnap) => {
            if (docSnap.exists()) {
                ...
                const hydratedProducts = subcollectionProducts.length > 0 ? subcollectionProducts : (cloudData.products || []);
                const hydratedCustomers = subcollectionCustomers.length > 0 ? subcollectionCustomers : (cloudData.customers || []);
                const hydratedTransactions = subcollectionTransactions.length > 0 ? subcollectionTransactions : (cloudData.transactions || []);
                memoryState = {
                    ...initialData,
                    ...cloudData,
...
export const deleteProduct = async (id: string): Promise<Product[]> => {
  ...
  await saveData({ ...data }, { throwOnError: true, reason: 'deleteProduct_metadata', auditOperation: 'UPDATE' });
...
export const addCustomer = (customer: Customer): Customer[] => {
    ...
    void upsertCustomerInSubcollection(newCustomer, 'addCustomer')
      .then(() => saveData({ ...data }, { reason: 'addCustomer_metadata', auditOperation: 'UPDATE' }))
      ...
      .catch(error => console.error('[phase1-customers] add customer failed', error));
...
export const updateCustomer = (customer: Customer): Customer[] => {
    ...
    void upsertCustomerInSubcollection(customer, 'updateCustomer')
      .then(() => saveData({ ...data }, { reason: 'updateCustomer_metadata', auditOperation: 'UPDATE' }))
      .then(() => writeAuditEvent('UPDATE', {
...
export const deleteCustomer = (id: string): Customer[] => {
    ...
    void deleteCustomerInSubcollection(id, 'deleteCustomer')
      .then(() => saveData({ ...data }, { reason: 'deleteCustomer_metadata', auditOperation: 'UPDATE' }))
  ...
  if (db) {
    void commitProcessTransactionAtomically({
      ...
      .then(({ created, committedProducts, committedCustomer }) => {
        if (!created) {
          return writeAuditEvent('BLOCKED_WRITE', {
        ...
        if (committedProducts.length > 0 || committedCustomer) {
        ...
        ]).then(() => saveData({ ...data }, { reason: 'processTransaction_metadata', auditOperation: 'UPDATE' }));
      })
      .catch(error => {
        console.error('[phase1-transactions] failed atomic processTransaction commit', error);
...
export const deleteTransaction = (transactionId: string): Transaction[] => {
  ...
  void deleteTransactionInSubcollection(transactionId, 'deleteTransaction')
    .then(() => saveData({ ...data }, { reason: 'deleteTransaction_metadata', auditOperation: 'UPDATE' }))
    .then(() => writeAuditEvent('DELETE', {
