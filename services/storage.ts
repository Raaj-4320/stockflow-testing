...
export const updateTransaction = async (updatedTransaction: Transaction): Promise<Transaction[]> => {
  ...
  const originalDueHint = toFiniteNonNegative(data.customers.find(c => c.id === originalTransaction.customerId)?.totalDue);
  const updatedDueHint = toFiniteNonNegative(stateWithoutOriginal.customers.find(c => c.id === effectiveUpdatedTransaction.customerId)?.totalDue);
  const originalCashbookEffect = getTransactionCashbookEffectSnapshot(originalTransaction, originalHistorical, originalDueHint);
  const updatedCashbookEffect = getTransactionCashbookEffectSnapshot(effectiveUpdatedTransaction, updatedHistorical, updatedDueHint);
