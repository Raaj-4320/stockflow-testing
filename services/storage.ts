...
const getReturnReconciliationAmounts = (
  ...
  if (mode === 'refund_online') {
    if (mixedPaidCreditReturn) {
      ...
      const onlineRefund = roundCurrency(Math.min(afterDue, caps.maxOnlineRefund));
      const remainder = roundCurrency(Math.max(0, afterDue - onlineRefund));
  ...
  if (mixedPaidCreditReturn) {
    const dueReduction = roundCurrency(Math.min(toFiniteNonNegative(dueBefore), caps.maxDueReduction, validReturnValue));
    const afterDue = roundCurrency(Math.max(0, validReturnValue - dueReduction));
    const cashRefund = roundCurrency(Math.min(afterDue, caps.maxCashRefund));
    const remainder = roundCurrency(Math.max(0, afterDue - cashRefund));
