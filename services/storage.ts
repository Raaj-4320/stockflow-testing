...
const sanitizeFreightImageFields = <T extends Record<string, any>>(entity: T): T => {
  ...
  if (typeof next.productPhoto === 'string' && next.productPhoto.startsWith('data:image')) next.productPhoto = '';
  if (typeof next.image === 'string' && next.image.startsWith('data:image')) next.image = '';
  if (typeof next.imageUrl === 'string' && next.imageUrl.startsWith('data:image')) next.imageUrl = '';
  return next as T;
};
...
export const convertInquiryToConfirmedOrder = async (
  ...
  const baseOrder: FreightConfirmedOrder = {
    ...
    productPhoto: sanitizeFreightImageFields(inquiry).productPhoto,
    productName: inquiry.productName,
    variant: inquiry.variant,
    color: inquiry.color,
    category: inquiry.category,
...
export const receiveFreightPurchaseIntoInventory = async (purchaseId: string): Promise<{ purchase: FreightPurchase; product: Product }> => {
  ...
  const product: Product = {
    ...
    sellPrice,
    stock: qty,
    image: (sanitizeFreightImageFields(purchase as any).productPhoto as string) || '',
