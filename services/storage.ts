...
export const saveData = async (data: AppState, options?: { throwOnError?: boolean; allowDestructive?: boolean; reason?: string }) => {
  const suspicious = isSuspiciousDrop(memoryState, data);
...
export const deleteProduct = async (id: string): Promise<Product[]> => {
  ...
  await saveData({ ...data, products: newProducts }, { throwOnError: true, reason: 'deleteProduct' });
  await writeAuditEvent('DELETE', { entity: 'product', id, beforeCount: data.products.length, afterCount: newProducts.length });
  return newProducts;
};
...
export const addCustomer = (customer: Customer): Customer[] => {
    ...
    void saveData({ ...data, customers: newCustomers }, { reason: 'deleteCustomer' });
    void writeAuditEvent('DELETE', { entity: 'customer', id, beforeCount: data.customers.length, afterCount: newCustomers.length });
    return newCustomers;
}
