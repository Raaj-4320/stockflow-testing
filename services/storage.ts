...
const shouldStoreCreditTrace = (transaction?: Partial<Transaction> | null, customer?: Partial<Customer> | null) => {
  const created = Number((transaction as any)?.storeCreditCreated || 0);
  const txName = String((transaction as any)?.customerName || '').toLowerCase();
  const customerName = String(customer?.name || '').toLowerCase();
  const txPhone = String((transaction as any)?.customerPhone || '').trim();
  const customerPhone = String(customer?.phone || '').trim();
  return created > 0 || txName.includes('ruchit') || customerName.includes('ruchit') || txPhone === '9726759110' || customerPhone === '9726759110';
};
...
const commitProcessTransactionAtomically = async ({
  ...
  return runFirestoreTransaction(db!, async (firestoreTx) => {
    ...
    if (transaction.customerId) {
      ...
      if (shouldStoreCreditTrace(transaction, currentCustomer)) {
        console.log("[STORE CREDIT TRACE]", JSON.stringify({
          stage: "storage_before_customer_update",
          customerId: currentCustomer.id,
          beforeStoreCredit: currentCustomer.storeCredit || 0,
          beforeTotalDue: currentCustomer.totalDue || 0,
          incomingStoreCreditCreated: storeCreditCreated,
          incomingStoreCreditUsed: storeCreditUsed,
          computedAfterStoreCredit: storeCredit,
          computedAfterTotalDue: totalDue
        }, null, 2));
      }
      committedCustomer = {
        ...currentCustomer,
      ...
      if (shouldStoreCreditTrace(transaction, currentCustomer)) {
        console.log("[STORE CREDIT TRACE]", JSON.stringify({
          stage: "storage_after_customer_update_payload",
          customerId: currentCustomer.id,
          updatePayloadStoreCredit: committedCustomer.storeCredit || 0,
          updatePayloadTotalDue: committedCustomer.totalDue || 0,
          updatePayloadKeys: Object.keys(committedCustomer || {})
        }, null, 2));
      }
    ...
    if (shouldStoreCreditTrace(transaction)) {
      console.log("[STORE CREDIT TRACE]", JSON.stringify({
        stage: "storage_before_transaction_write",
        txId: transaction.id,
        transactionPayloadStoreCreditCreated: transaction.storeCreditCreated || 0,
        transactionPayloadStoreCreditUsed: transaction.storeCreditUsed || 0,
        transactionPayloadKeys: Object.keys(transaction || {})
      }, null, 2));
    }
...
export const processTransaction = (transaction: Transaction): AppState => {
  if (shouldStoreCreditTrace(transaction)) {
    console.log("[STORE CREDIT TRACE]", JSON.stringify({
      stage: "storage_processTransaction_start",
      txId: transaction.id,
      type: transaction.type,
      customerId: transaction.customerId || null,
      total: transaction.total,
      storeCreditCreated: transaction.storeCreditCreated || 0,
      storeCreditUsed: transaction.storeCreditUsed || 0,
      saleSettlement: transaction.saleSettlement || null
    }, null, 2));
  }
  let data = ensureDocumentSeriesDefaults(loadData());
