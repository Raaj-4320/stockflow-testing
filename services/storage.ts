...
export const isHistoricalTransaction = (transaction: Transaction) => transaction.mode === 'historical';
...
export const processTransaction = (transaction: Transaction): AppState => {
  const data = loadData();
  const normalizedTransaction = prepareTransactionRecord({
    ...transaction,
  ...
  if (!normalizedTransaction || typeof normalizedTransaction !== 'object') {
    failValidation('INVALID_TRANSACTION_PAYLOAD', 'Transaction payload is invalid.');
  }

  if (!normalizedTransaction.id || !normalizedTransaction.date) {
  ...
  if (normalizedTransaction.mode === 'historical') {
    const historicalState = {
      ...data,
      transactions: sortTransactionsDesc([normalizedTransaction, ...data.transactions]),
    };
    void addHistoricalTransactions([normalizedTransaction]);
    return historicalState;
  }

  assertPaymentMethodByType(normalizedTransaction.type, normalizedTransaction.paymentMethod);
  assertTransactionFinancials(normalizedTransaction);
  assertTransactionInventoryRules(normalizedTransaction, data.products, getOperationalTransactions(data.transactions));

  const newTransactions = [normalizedTransaction, ...data.transactions];
  let newProducts = [...data.products];
  if (normalizedTransaction.type !== 'payment') {
      newProducts = data.products.map(p => applyTransactionItemsToProduct(p, normalizedTransaction.items, normalizedTransaction.type));
  }
  let newCustomers = [...data.customers];
  if (normalizedTransaction.customerId) {
      const customerIndex = newCustomers.findIndex(c => c.id === normalizedTransaction.customerId);
      if (customerIndex === -1) {
        failValidation('CUSTOMER_NOT_FOUND', 'Transaction customer not found.', { customerId: normalizedTransaction.customerId });
      }
      ...
      const amount = Math.abs(normalizedTransaction.total);
      if (normalizedTransaction.type === 'sale') {
          newTotalSpend += amount;
          newVisitCount += 1;
          newLastVisit = new Date().toISOString();
          if (normalizedTransaction.paymentMethod === 'Credit') newTotalDue += amount;
      } else if (normalizedTransaction.type === 'return') {
          newTotalSpend -= amount;
          if (normalizedTransaction.paymentMethod === 'Credit') newTotalDue -= amount;
      } else if (normalizedTransaction.type === 'payment') {
          newTotalDue -= amount;
          newLastVisit = new Date().toISOString();
      }
  ...
  const touchedProductIds = normalizedTransaction.type !== 'payment'
  ...
  if (normalizedTransaction.customerId && normalizedTransaction.type !== 'payment') {
    touchedProductIds.forEach((productId) => {
      const soldQty = data.transactions
        .filter(t => t.customerId === normalizedTransaction.customerId && t.type === 'sale')
      ...
      const returnedQty = data.transactions
        .filter(t => t.customerId === normalizedTransaction.customerId && t.type === 'return')
        .reduce((acc, t) => acc + t.items.filter(i => i.id === productId).reduce((itemSum, item) => itemSum + (item.quantity || 0), 0), 0);
    ...
    void commitProcessTransactionAtomically({
      ...
      .then(({ created, committedProducts, committedCustomer }) => {
        ...
        const nextTransactions = memoryState.transactions.some(t => t.id === normalizedTransaction.id)
          ...
          : [normalizedTransaction, ...memoryState.transactions];
        memoryState = {
          ...memoryState,
...
export const addHistoricalTransactions = async (transactions: Transaction[]): Promise<Transaction[]> => {
  ...
  const data = loadData();
  const normalizedIncoming = transactions.map(tx => prepareTransactionRecord({
    ...tx,
    mode: 'historical',
