...
const applyBucketDeltaToProduct = (
  ...
  const stockDelta = transactionType === 'sale'
    ? -quantity
    : transactionType === 'return'
      ...
      : transactionType === 'purchase'
        ? quantity
        : quantity;
  const soldDelta = transactionType === 'sale'
    ? quantity
    : transactionType === 'return'
      ? -quantity
      : 0;
  const purchaseDelta = transactionType === 'purchase' ? quantity : 0;

  if (!hasVariantRows) {
    return {
      ...product,
      stock: Math.max(0, (product.stock || 0) + stockDelta),
      totalSold: Math.max(0, (product.totalSold || 0) + soldDelta),
      totalPurchase: Math.max(0, (product.totalPurchase || 0) + purchaseDelta),
    };
  }
  ...
  if (rowIndex >= 0) {
    rows[rowIndex] = {
      ...
      totalSold: Math.max(0, (rows[rowIndex].totalSold || 0) + soldDelta),
      totalPurchase: Math.max(0, (rows[rowIndex].totalPurchase || 0) + purchaseDelta),
    };
    ...
    rows.push({
      ...
      totalSold: Math.max(0, soldDelta),
      totalPurchase: Math.max(0, purchaseDelta),
    });
  }
