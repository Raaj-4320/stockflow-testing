import { getWhatsAppServerUrl } from './whatsapp';

const getBaseUrl = () => {
  const url = ((import.meta as any)?.env?.VITE_WHATSAPP_SERVER_URL || '').trim();
  if (!url) throw new Error('WhatsApp server URL is not configured.');
  return url.replace(/\/$/, '');
};
...
export const getResolvedLegacyWhatsAppServerUrl = () => getWhatsAppServerUrl();
