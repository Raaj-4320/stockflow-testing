...
export const generateReceiptPDF = (transaction: Transaction, customers: Customer[], paymentDetails?: ReceiptPaymentDetails) => {
    ...
    const logoX = 14;
    const logoY = 10;
    const logoBoxW = 31.2; // ~20% larger
    const logoBoxH = 19.2; // ~20% larger
    if (logoData) {
      try {
        const props = (doc as any).getImageProperties(logoData);
    ...
    const headerBottomY = Math.max(logoY + logoBoxH, headerLinesStartY + (Math.max(0, headerLines.length - 1) * 4.2), 20) + 6;
    ...
    doc.rect(totalsX - 53, summaryY, 53, 8, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFont("helvetica", "bold");
