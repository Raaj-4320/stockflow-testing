...
export const generateProductCatalogPDF = async (
    ...
    const cardPadding = 3;
    const imageBlockHeight = Math.max(16, Math.min(cardHeight * 0.46, 30));
    ...
    for (let categoryIndex = 0; categoryIndex < sortedCategories.length; categoryIndex += 1) {
        ...
        for (let offset = 0; offset < categoryProducts.length; offset += cardsPerPage) {
            ...
            for (let i = 0; i < chunk.length; i += 1) {
                ...
                const nameRaw = (product.name || '').trim() || 'Unnamed product';
                ...
                let textY = imageY + imageBoxSize + 4;
                doc.setFont('helvetica', 'bold');
                doc.setFontSize(8.5);
                ...
                const priceText = `Price: Rs.${Number.isFinite(product.sellPrice) ? product.sellPrice.toFixed(2) : '0.00'}`;
                const stockText = `Stock: ${Number.isFinite(product.stock) ? product.stock : 0}`;
                doc.text(`Barcode: ${barcode}`, textX, textY);
                textY += 4;
                doc.text(priceText, textX, textY);
                textY += 4;
                doc.text(stockText, textX, textY);
            }
        }
    }
