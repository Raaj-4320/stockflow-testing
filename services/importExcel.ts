...
const writeTemplate = (sheetName: string, headers: string[], sample: any[], fields: TemplateField[]) => {
  ...
  const wsInfo = XLSX.utils.aoa_to_sheet([
    ['Field', 'Requirement', 'Format', 'Notes', 'Example'],
    ...fields.map(f => [f.field, f.required, f.format, f.notes, f.example]),
  ]);
...
export const downloadInventoryTemplate = () => writeTemplate(
  ...
  [
    { field: 'Product ID', required: 'Preferred', format: 'Text', notes: 'Stable identity for update matching. Keep unchanged for existing records.', example: 'product-001' },
    { field: 'Barcode', required: 'Mandatory', format: 'Text', notes: 'Must be unique. Duplicate barcode rows are rejected.', example: 'SKU-1001' },
    { field: 'Product Name', required: 'Mandatory', format: 'Text', notes: 'Product display name.', example: 'Cotton Shirt' },
    { field: 'Category', required: 'Mandatory', format: 'Text', notes: 'Will be auto-created if not present.', example: 'Apparel' },
    { field: 'Buy Price', required: 'Mandatory', format: 'Number >= 0', notes: 'Cost price.', example: '250' },
    { field: 'Sell Price', required: 'Mandatory', format: 'Number >= 0', notes: 'Selling price.', example: '499' },
    { field: 'Stock', required: 'Mandatory', format: 'Number >= 0', notes: 'Opening stock quantity.', example: '20' },
    { field: 'HSN/SAC', required: 'Optional', format: 'Text', notes: 'Tax code.', example: '6109' },
    { field: 'Image Source', required: 'Optional', format: 'Cloudinary URL | public https image URL | data:image base64', notes: 'Public URL is fetched and uploaded to Cloudinary. Local file paths are not supported.', example: 'https://example.com/images/shirt.jpg' },
    { field: 'Description', required: 'Optional', format: 'Text', notes: 'Product notes.', example: 'Regular fit cotton shirt' },
  ]
);
...
export const downloadCustomersTemplate = () => writeTemplate(
  ...
  [
    { field: 'Customer ID', required: 'Preferred', format: 'Text', notes: 'Stable identity for update matching. Keep unchanged for existing records.', example: 'customer-001' },
    { field: 'Name', required: 'Mandatory', format: 'Text', notes: 'Customer display name.', example: 'Ravi Kumar' },
    { field: 'Phone', required: 'Mandatory', format: 'Text (8+ digits)', notes: 'Must be unique. Duplicate phones are rejected.', example: '9876543210' },
    { field: 'Total Spend', required: 'Optional', format: 'Number >= 0', notes: 'Opening spend balance only. Future spend is transaction-derived.', example: '5000' },
    { field: 'Total Due', required: 'Optional', format: 'Number >= 0', notes: 'Opening due balance.', example: '750' },
    { field: 'Opening Credit', required: 'Optional', format: 'Number >= 0', notes: 'Alias for opening due/credit balance. Used only if Total Due is empty.', example: '750' },
    { field: 'Visit Count', required: 'Optional', format: 'Integer >= 0', notes: 'Opening visit count.', example: '4' },
    { field: 'Last Visit (ISO DateTime)', required: 'Preferred', format: 'ISO datetime', notes: 'If blank, current date-time is used.', example: new Date().toISOString() },
    { field: '[Derived] Purchase history (what purchased/date-time)', required: 'Preferred', format: 'System derived from transactions', notes: 'Do not import as master data; this is built from transaction records.', example: 'Auto-derived' },
    { field: '[Derived] Running total spend/credit', required: 'Preferred', format: 'System derived from transactions + opening balances', notes: 'Imported opening values are seeds, ongoing values are transaction-driven.', example: 'Auto-updated' },
  ]
);

export const downloadTransactionsTemplate = () => writeTemplate(
  'Transactions',
  ['Transaction ID', 'Date', 'Type', 'Customer Phone', 'Customer Name', 'Payment Method', 'Product Barcode', 'Variant', 'Color', 'Quantity', 'Unit Sell Price', 'Item Discount', 'Tax Rate', 'Tax Label', 'Subtotal', 'Discount', 'Tax', 'Total', 'Amount', 'Notes'],
  ['TX-1001', new Date().toISOString(), 'sale', '9876543210', 'Ravi Kumar', 'Cash', 'SKU-1001', NO_VARIANT, NO_COLOR, 2, 499, 0, 0, 'GST', 998, 0, 0, 998, '', 'sample sale import'],
  [
    { field: 'Transaction ID', required: 'Mandatory', format: 'Text', notes: 'Unique ID per transaction. Same ID groups multiple item rows.', example: 'TX-1001' },
    { field: 'Date', required: 'Mandatory', format: 'ISO or parseable date-time', notes: 'Transaction date/time.', example: new Date().toISOString() },
    { field: 'Type', required: 'Mandatory', format: 'sale | return | payment', notes: 'payment uses Amount field; sale/return uses item rows.', example: 'sale' },
    { field: 'Customer Phone', required: 'Preferred', format: 'Text digits', notes: 'Used to map existing customer.', example: '9876543210' },
    { field: 'Customer Name', required: 'Optional', format: 'Text', notes: 'Used as fallback lookup if phone missing. Must uniquely match existing customer.', example: 'Ravi Kumar' },
    { field: 'Payment Method', required: 'Mandatory', format: 'Cash | Credit | Online', notes: 'Credit requires matched customer.', example: 'Cash' },
    { field: 'Product Barcode', required: 'Mandatory', format: 'Text', notes: 'Must exist in inventory.', example: 'SKU-1001' },
    { field: 'Quantity', required: 'Mandatory', format: 'Number > 0', notes: 'Stock checks apply.', example: '2' },
    { field: 'Unit Sell Price', required: 'Mandatory', format: 'Number >= 0', notes: 'Line unit sell price.', example: '499' },
    { field: 'Item Discount', required: 'Optional', format: 'Number >= 0', notes: 'Per-line discount amount.', example: '0' },
    { field: 'Tax Rate', required: 'Optional', format: 'Number >= 0', notes: 'Defaults to 0 if empty.', example: '0' },
    { field: 'Tax Label', required: 'Optional', format: 'Text', notes: 'Tax label metadata.', example: 'GST' },
    { field: 'Subtotal/Discount/Tax/Total', required: 'Optional', format: 'Numbers', notes: 'If provided, validated against computed values.', example: '998 / 0 / 0 / 998' },
    { field: 'Amount', required: 'Mandatory', format: 'Number > 0', notes: 'Payment amount.', example: '500' },
    { field: 'Notes', required: 'Optional', format: 'Text', notes: 'Additional transaction note.', example: 'sample sale import' },
  ]
);
...
export const downloadPurchaseTemplate = () => writeTemplate(
  ...
  [
    { field: 'Order ID', required: 'Mandatory', format: 'Text', notes: 'Unique purchase order id.', example: 'PO-1001' },
    { field: 'Order Date', required: 'Mandatory', format: 'ISO or parseable date-time', notes: 'Order date.', example: new Date().toISOString() },
    { field: 'Party Name', required: 'Mandatory', format: 'Text', notes: 'Must match an existing purchase party.', example: 'ABC Suppliers' },
    { field: 'Source Type', required: 'Mandatory', format: 'inventory | new', notes: 'inventory requires Product Barcode; new requires Product Name.', example: 'inventory' },
    { field: 'Product Barcode', required: 'Preferred', format: 'Text', notes: 'Required when Source Type = inventory.', example: 'SKU-1001' },
    { field: 'Product Name', required: 'Preferred', format: 'Text', notes: 'Required when Source Type = new.', example: 'New Fabric Roll' },
    { field: 'Quantity', required: 'Mandatory', format: 'Number > 0', notes: 'Line quantity.', example: '10' },
    { field: 'Unit Cost', required: 'Mandatory', format: 'Number > 0', notes: 'Line unit cost.', example: '220' },
  ]
);
...
export const downloadInventoryData = () => {
  ...
  writeDataWorkbook('Inventory', rows, [
    { field: 'Product ID', required: 'Preferred', format: 'Text', notes: 'Primary update identity.', example: 'product-001' },
    { field: 'Barcode', required: 'Mandatory', format: 'Text', notes: 'Unique product barcode.', example: 'SKU-1001' },
    { field: 'Image Source', required: 'Optional', format: 'Cloudinary URL | public https image URL | data:image base64', notes: 'On upload, supported images are saved as Cloudinary URLs.', example: 'https://res.cloudinary.com/.../image/upload/...' },
  ], 'Inventory_Data');
};
...
export const downloadCustomersData = () => {
  ...
  writeDataWorkbook('Customers', rows, [
    { field: 'Customer ID', required: 'Preferred', format: 'Text', notes: 'Primary update identity.', example: 'customer-001' },
    { field: 'Phone', required: 'Mandatory', format: 'Text', notes: 'Unique customer phone fallback identity.', example: '9876543210' },
    { field: '[Derived] Purchase history', required: 'Preferred', format: 'System derived', notes: 'Not directly edited in customer sheet; driven from transactions.', example: 'Auto-derived' },
  ], 'Customers_Data');
};
...
export const downloadTransactionsData = () => {
  ...
  writeDataWorkbook('Transactions', rows, [
    { field: 'Transaction ID', required: 'Mandatory', format: 'Text', notes: 'Stable immutable transaction identity.', example: 'TX-1001' },
    { field: 'Type', required: 'Mandatory', format: 'sale|return|payment', notes: 'Transaction updates are strict; mismatched payloads are rejected.', example: 'sale' },
  ], 'Transactions_Data');
};
...
export const downloadPurchaseData = () => {
  ...
  writeDataWorkbook('Purchase', rows, [
    { field: 'Order ID', required: 'Mandatory', format: 'Text', notes: 'Stable purchase order identity used for update matching.', example: 'PO-1001' },
    { field: 'Party Name', required: 'Mandatory', format: 'Text', notes: 'Must match existing party.', example: 'ABC Suppliers' },
  ], 'Purchase_Data');
};
...
export const importTransactionsFromFile = async (file: File, onProgress?: (progress: ImportProgress) => void): Promise<ImportResult> => {
  ...
  for (const [txId, txRows] of grouped.entries()) {
    ...
    let customer = customerPhone ? customersByPhone.get(customerPhone) : undefined;
    if (!customer && customerNameFromFile) {
      ...
      if (byName.length > 1) errors.push({ sheet: 'Transactions', row: rowNo0, field: 'Customer Name', message: 'Multiple customers match this name. Provide Customer Phone.' });
    }
    ...
    if (paymentMethod === 'Credit' && !customer) errors.push({ sheet: 'Transactions', row: rowNo0, field: 'Customer', message: 'Credit transactions require an existing customer (phone/name match)' });
    if (customerNameFromFile && customer && normName(customer.name) !== normName(customerNameFromFile)) {
      errors.push({ sheet: 'Transactions', row: rowNo0, field: 'Customer Name', message: 'Customer Name does not match resolved customer for provided phone' });
    }
