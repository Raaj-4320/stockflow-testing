...
export const downloadInventoryTemplate = () => writeTemplate(
  'Inventory',
  ['Barcode', 'Product Name', 'Category', 'Buy Price', 'Sell Price', 'Stock', 'HSN/SAC', 'Image Source', 'Description'],
  ['SKU-1001', 'Cotton Shirt', 'Apparel', 250, 499, 20, '6109', 'https://example.com/images/shirt.jpg', 'Regular fit cotton shirt'],
  [
...
export const downloadCustomersTemplate = () => writeTemplate(
  'Customers',
  ['Name', 'Phone', 'Total Spend', 'Total Due', 'Opening Credit', 'Visit Count', 'Last Visit (ISO DateTime)'],
  ['Ravi Kumar', '9876543210', 5000, 750, '', 4, new Date().toISOString()],
  [
...
export const importCustomersFromFile = async (file: File, onProgress?: (progress: ImportProgress) => void): Promise<ImportResult> => {
  ...
  rows.forEach((row, i) => {
    ...
    if (normalizedPhone && existingPhones.has(normalizedPhone)) errors.push({ sheet: 'Customers', row: rowNo, field: 'Phone', message: 'Customer phone already exists' });
...
export const importTransactionsFromFile = async (file: File, onProgress?: (progress: ImportProgress) => void): Promise<ImportResult> => {
  ...
  for (const [txId, txRows] of grouped.entries()) {
    ...
    if (existingTxIds.has(txId)) {
      errors.push({ sheet: 'Transactions', row: rowNo0, field: 'Transaction ID', message: 'Transaction ID already exists' });
      continue;
    }
