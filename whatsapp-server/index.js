...
async function sendDocumentToWhatsApp({ userId, customerPhone, customerName, documentNo, pdfUrl, type }) {
  ...
  const media = await downloadMediaFromUrl(pdfUrl);
...
app.post('/send-invoice', requireApiKey, async (req, res) => {
  try {
    ...
    const sent = await sendDocumentToWhatsApp({ userId, customerPhone, customerName, documentNo: invoiceNo, pdfUrl, type: 'invoice' });
...
app.post('/send-ledger', requireApiKey, async (req, res) => {
  try {
    ...
    if (!userId || !customerPhone || !pdfUrl) return res.status(400).json({ success: false, error: 'userId, customerPhone, pdfUrl required' });
    const sent = await sendDocumentToWhatsApp({ userId, customerPhone, customerName, documentNo: ledgerNo, pdfUrl, type: 'ledger' });
    const m = getMetric(String(userId));
