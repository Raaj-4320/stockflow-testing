...
app.post('/send-invoice', requireApiKey, upload.single('pdf'), async (req, res) => {
  try {
    ...
    if (!userId || !customerPhone || !customerName) return res.status(400).json({ success: false, error: 'userId, customerPhone, customerName required' });
    const media = await buildMediaFromRequest(req);
    if (!media && !String(pdfUrl || '').trim()) return res.status(400).json({ success: false, error: 'pdf file or pdfUrl required' });
