...
import { can, ENABLE_ROLE_ACCESS, PermissionKey, restrictedRoutes } from './src/auth/permissions';
...
const ProtectedRoute = ({ isVerified, permission, children }: { isVerified: boolean; permission?: PermissionKey; children: React.ReactElement }) => {
...
function AppContent() {
  ...
  const { session: roleSession, setSession: setRoleSession, logoutRole } = useRoleSession();
  ...
  const showNav = (path: string) => !restrictedRoutes[path] || can(roleSession, restrictedRoutes[path]);
  ...
  return (
    <Router>
      ...
      <div className="flex h-screen bg-background overflow-hidden">
        ...
        {ENABLE_ROLE_ACCESS && authStatus === 'authenticated' && !roleSession && <RoleLoginModal onLogin={setRoleSession} />}
        {/* Sidebar */}
        <div className="w-64 border-r bg-card flex flex-col hidden md:flex">
          ...
          <nav className="flex-1 px-4 space-y-1">
            ...
            {can(roleSession, 'viewProfitAnalytics') && <NavItem to="/pdf" icon={FileText} label="Reports" />}
          ...
          <div className="p-4 border-t flex flex-col gap-2">
             <div className="text-xs text-muted-foreground mt-2">
                <p>User: {currentEmail}</p><p>Access: {!ENABLE_ROLE_ACCESS ? 'Admin (default)' : roleSession?.role === 'operator' ? `Operator (${roleSession.operatorName})` : roleSession?.role || 'Not selected'}</p>
             </div>
            ...
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60] flex flex-col justify-end animate-in slide-in-from-bottom-10" onClick={() => setIsMenuOpen(false)}>
                <div className="bg-card rounded-t-2xl p-6 space-y-4 pb-8" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                         ...
                         {can(roleSession, 'viewProfitAnalytics') && <Link to="/pdf" className="flex flex-col items-center justify-center p-4 bg-muted/50 rounded-xl hover:bg-muted transition-colors border border-transparent hover:border-primary/20">
          ...
          <div className="h-full p-4 md:p-8 pb-20 md:pb-8 max-w-7xl mx-auto">
            ...
            <Suspense fallback={<div className="h-full flex items-center justify-center text-sm text-muted-foreground">Loading page…</div>}>
              <Routes>
                ...
                <Route path="/product-analytics" element={<ProtectedRoute isVerified={authStatus === "authenticated"} permission="accessProductAnalytics"><ProductAnalytics /></ProtectedRoute>} />
                <Route path="/customers" element={<ProtectedRoute isVerified={authStatus === "authenticated"}><Customers /></ProtectedRoute>} />
                <Route path="/pdf" element={<ProtectedRoute isVerified={authStatus === "authenticated"} permission="viewProfitAnalytics"><Reports /></ProtectedRoute>} />
                <Route path="/settings" element={<ProtectedRoute isVerified={authStatus === "authenticated"} permission="manageSettings"><Settings /></ProtectedRoute>} />
                <Route path="/whatsapp-logs" element={<ProtectedRoute isVerified={authStatus === "authenticated"} permission="manageSettings"><WhatsAppLogs /></ProtectedRoute>} />
                <Route path="/cashbook" element={<ProtectedRoute isVerified={authStatus === "authenticated"} permission="viewCashbook"><Cashbook /></ProtectedRoute>} />
                <Route path="/finance" element={<ProtectedRoute isVerified={authStatus === "authenticated"}><Finance /></ProtectedRoute>} />
                <Route path="/freight-booking" element={<ProtectedRoute isVerified={authStatus === "authenticated"} permission="accessFreight"><FreightBooking /></ProtectedRoute>} />
                <Route path="/purchase-panel" element={<ProtectedRoute isVerified={authStatus === "authenticated"} permission="accessPurchases"><PurchasePanel /></ProtectedRoute>} />
