...
export default function App() {
  ...
  useEffect(() => {
    const onHashChange = () => {
      ...
      if (!pageKey || !settings.protectedPages.includes(pageKey) || !settings.adminPasscode) return;
      if (unlockIdentity?.type === 'admin') return;
      if (unlockIdentity?.type === 'role') {
        const role = settings.roles.find(r => r.id === unlockIdentity.roleId);
        if (role?.allowedPages?.includes(pageKey)) return;
      }
      setPasscodeInput('');
      setPasscodeError('');
