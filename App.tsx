...
export default function App() {
  ...
  const [unlockNavigationTarget, setUnlockNavigationTarget] = useState<string | null>(null);
  ...
  useEffect(() => {
    const onHashChange = () => {
      ...
      if (unlockNavigationTarget && path === unlockNavigationTarget) {
        setLastAllowedPath(path);
        setUnlockNavigationTarget(null);
        return;
      }
  ...
  }, [unlockIdentity, lastAllowedPath, unlockNavigationTarget]);
  ...
  useEffect(() => {
    ...
    if (passcodeInput === settings.adminPasscode && settings.adminPasscode) {
      ...
      setUnlockNavigationTarget(accessPrompt.path);
  ...
  useEffect(() => {
    if (!unlockNavigationTarget || !unlockIdentity) return;
    if ((window.location.hash.replace('#', '') || '/') !== unlockNavigationTarget) {
      window.location.hash = unlockNavigationTarget;
    }
  }, [unlockNavigationTarget, unlockIdentity]);
