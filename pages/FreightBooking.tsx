...
const calcOrderNumbers = (f: Pick<InquiryFormState, 'piecesPerCartoon' | 'numberOfCartoons' | 'rmbPricePerPiece' | 'inrPricePerPiece' | 'exchangeRate' | 'cbmPerCartoon' | 'cbmRate' | 'sellingPrice'>) => {
...
export default function FreightBooking() {
  ...
  const validate = () => {
    ...
    if (form.variantSelectionMode === 'exact') {
      ...
      } else if (selectedLines.some(line => line.totalPieces <= 0)) {
      ...
      } else if (selectedLines.some(line => line.rmbPricePerPiece <= 0 && line.inrPricePerPiece <= 0)) {
    ...
    const values = [
      form.piecesPerCartoon, form.numberOfCartoons, form.rmbPricePerPiece, form.inrPricePerPiece, form.exchangeRate, form.cbmPerCartoon, form.cbmRate,
  ...
  const toggleLineSelection = (id: string, checked: boolean) => setLineItems(prev => prev.map(l => l.id === id ? { ...l, selected: checked } : l));
  ...
  const saveInquiry = async (status: 'draft' | 'saved') => {
    ...
    const exactLines = mode === 'exact' ? selectedLines.map(line => toLineSnapshot(line, form)) : [];
    ...
    const payload: FreightInquiry = {
      ...
      totalPieces: mode === 'exact' ? to2(aggregatedFromLines.totalPieces) : form.totalPieces,
      ...
      totalRmb: mode === 'exact' ? to2(aggregatedFromLines.totalRmb) : form.totalRmb,
      inrPricePerPiece: mode === 'exact' ? (form.pricingMode === 'common' ? form.inrPricePerPiece : 0) : form.inrPricePerPiece,
      totalInr: mode === 'exact' ? to2(aggregatedFromLines.totalInr) : form.totalInr,
      exchangeRate: form.exchangeRate,
      freightPerCbm: 0,
      cbmPerCartoon: form.cbmPerCartoon,
      totalCbm: mode === 'exact' ? to2(aggregatedFromLines.totalCbm) : form.totalCbm,
      cbmRate: form.cbmRate,
      cbmCost: mode === 'exact' ? to2(aggregatedFromLines.cbmCost) : form.cbmCost,
      cbmPerPiece: mode === 'exact' && aggregatedFromLines.totalPieces > 0 ? to2(aggregatedFromLines.cbmCost / aggregatedFromLines.totalPieces) : form.cbmPerPiece,
      productCostPerPiece: mode === 'exact' && aggregatedFromLines.totalPieces > 0 ? to2(aggregatedFromLines.productCostTotalAmount / aggregatedFromLines.totalPieces) : form.productCostPerPiece,
      sellingPrice: form.sellingPrice,
  ...
  const modeLabel = (inquiry: FreightInquiry) => {
    if (inquiry.variantSelectionMode === 'exact') return 'Exact variants';
          ...
          <CardContent className="space-y-4">
            ...
            <div className="overflow-x-auto border rounded-lg">
              <table className="min-w-full text-sm">
                <thead className="bg-muted/30"><tr><th className="p-3 text-left">Inquiry ID</th><th className="p-3 text-left">Product</th><th className="p-3 text-left">Variant handling</th><th className="p-3 text-left">Total pieces</th><th className="p-3 text-left">Status</th><th className="p-3 text-right">Actions</th></tr></thead>
                <tbody>
                  {!filteredInquiries.length && <tr><td className="p-4 text-center text-muted-foreground" colSpan={6}>No inquiries yet</td></tr>}
            ...
            <CardContent className="space-y-5">
                ...
                <section className="border rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="sm:col-span-2">
                      ...
                      <div className={`mt-1 rounded-lg border-2 border-dashed p-4 text-sm ${draggingImage ? 'border-primary bg-primary/5' : 'border-border'}`} onDragOver={e => { e.preventDefault(); setDraggingImage(true); }} onDragLeave={() => setDraggingImage(false)} onDrop={onDropImage}>
              ...
              <section className="border rounded-lg p-4 space-y-3">
                ...
                <div className="grid gap-2 md:grid-cols-3 text-sm">
                  ...
                  <label className={`border rounded p-2 cursor-pointer ${form.variantSelectionMode === 'exact' ? 'border-primary bg-primary/5' : ''}`}><input type="radio" className="mr-2" checked={form.variantSelectionMode === 'exact'} onChange={() => patchForm({ variantSelectionMode: 'exact' })} />Select exact variants</label>
                ...
                {form.variantSelectionMode === 'exact' && (
                  <>
                      ...
                      <div className="grid gap-3 md:grid-cols-3 text-sm">
                        <div>
                          ...
                          <div className="mt-1 space-y-1">
                            ...
                            <label className="block"><input type="radio" className="mr-1" checked={form.quantityMode === 'line_level'} onChange={() => patchForm({ quantityMode: 'line_level' })} />Separate per variant</label>
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="mt-1 space-y-1">
                            ...
                            <label className="block"><input type="radio" className="mr-1" checked={form.pricingMode === 'line_wise'} onChange={() => patchForm({ pricingMode: 'line_wise' })} />Separate per variant</label>
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="mt-1 space-y-1">
                            ...
                            <label className="block"><input type="radio" className="mr-1" checked={form.freightMode === 'line_level'} onChange={() => patchForm({ freightMode: 'line_level' })} />Separate per variant</label>
                          </div>
                        </div>
                      </div>
                    </section>
                    ...
                    {(form.quantityMode === 'order_level' || form.pricingMode === 'common' || form.freightMode === 'order_level') && (
                      <section className="border rounded-lg p-3 space-y-3">
                        ...
                        <div className="grid gap-3 md:grid-cols-3">
                          ...
                          {form.freightMode === 'order_level' && <><div><Label>CBM/carton</Label><Input type="number" min="0" value={form.cbmPerCartoon} onChange={e => patchForm({ cbmPerCartoon: Number(e.target.value) || 0 })} /></div><div><Label>Total CBM</Label><Input value={form.totalCbm} readOnly /></div><div><Label>CBM rate</Label><Input type="number" min="0" value={form.cbmRate} onChange={e => patchForm({ cbmRate: Number(e.target.value) || 0 })} /></div></>}
                        </div>
                    ...
                    {(form.quantityMode === 'order_level' || form.pricingMode === 'common' || form.freightMode === 'order_level') && renderCalculationPreview({
                      totalPieces: form.quantityMode === 'order_level' ? form.totalPieces : aggregatedFromLines.totalPieces,
                      totalRmb: form.pricingMode === 'common' ? form.totalRmb : aggregatedFromLines.totalRmb,
                      totalInr: form.pricingMode === 'common' ? form.totalInr : aggregatedFromLines.totalInr,
                      totalCbm: form.freightMode === 'order_level' ? form.totalCbm : aggregatedFromLines.totalCbm,
                      cbmCost: form.freightMode === 'order_level' ? form.cbmCost : aggregatedFromLines.cbmCost,
                      productCostTotalAmount: aggregatedFromLines.productCostTotalAmount,
                    }, { title: 'Shared/order-level calculation preview' })}

                    <div className="overflow-x-auto border rounded">
                      <table className="min-w-[1200px] w-full text-xs">
                        <thead className="bg-muted/30"><tr><th className="p-2">Use</th><th className="p-2 text-left">Variant</th><th className="p-2 text-left">Color</th><th className="p-2 text-right">Stock</th><th className="p-2">Pieces/carton</th><th className="p-2">Cartons</th><th className="p-2">Total pcs</th><th className="p-2">RMB/pc</th><th className="p-2">Rate</th><th className="p-2">INR/pc</th><th className="p-2">Total INR</th><th className="p-2">CBM/carton</th><th className="p-2">Total CBM</th><th className="p-2">CBM cost</th><th className="p-2">Cost/pc</th><th className="p-2">Total Cost</th><th className="p-2">Actions</th></tr></thead>
                        <tbody>
                          {!lineItems.length && <tr><td className="p-2 text-muted-foreground" colSpan={17}>No combinations yet. Add a line below.</td></tr>}
                          {lineItems.map(line => (
                            <tr key={line.id} className="border-t">
                              ...
                              <td className="p-1"><Input type="number" min="0" disabled={form.quantityMode === 'order_level'} value={line.piecesPerCartoon} onChange={e => updateLine(line.id, { piecesPerCartoon: Number(e.target.value) || 0 })} /></td>
                              <td className="p-1"><Input type="number" min="0" disabled={form.quantityMode === 'order_level'} value={line.numberOfCartoons} onChange={e => updateLine(line.id, { numberOfCartoons: Number(e.target.value) || 0 })} /></td>
                              <td className="p-1"><Input type="number" min="0" value={line.totalPieces} disabled={form.quantityMode === 'line_level'} onChange={e => updateLine(line.id, { totalPieces: Number(e.target.value) || 0 })} /></td>
                              ...
                              <td className="p-1"><Input type="number" min="0" disabled={form.pricingMode === 'common'} value={line.exchangeRate} onChange={e => updateLine(line.id, { exchangeRate: Number(e.target.value) || 0 }, true)} /></td>
                              ...
                              <td className="p-1"><Input type="number" min="0" disabled={form.freightMode === 'order_level'} value={line.cbmPerCartoon} onChange={e => updateLine(line.id, { cbmPerCartoon: Number(e.target.value) || 0 })} /></td>
                              ...
                              <td className="p-1"><Button type="button" size="sm" variant="outline" onClick={() => removeLine(line.id)}>Remove</Button></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    </div>
                    {(form.source === 'new' || !lineItems.length) && <Button type="button" variant="outline" size="sm" onClick={addVariantLine}>Add variant/color line</Button>}
                    {renderCalculationPreview({
                      ...
                      productCostTotalAmount: aggregatedFromLines.productCostTotalAmount,
                    }, { title: 'Exact-mode totals preview' })}
                ...
                <>
                  ...
                  <section className="border rounded-lg p-4 space-y-3">
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      ...
                      <div><Label>CBM/carton</Label><Input type="number" min="0" value={form.cbmPerCartoon} onChange={e => patchForm({ cbmPerCartoon: Number(e.target.value) || 0 })} /></div>
                  ...
                  }, { title: 'Shared order calculation preview' })}
                </>
              )}
            ...
            <CardContent className="space-y-4 text-sm">
              <div className="grid gap-3 sm:grid-cols-[110px_1fr]">
                ...
                <div className="grid gap-1 sm:grid-cols-2">
                  ...
                  <div><span className="text-muted-foreground">Freight mode:</span> {summaryInquiry.freightMode || 'order_level'}</div>
                </div>
              </div>
      ...
      {showDetailsModal && selectedInquiry && (
        <div className="fixed inset-0 z-[70] bg-black/45 flex items-center justify-center p-4" onClick={() => setShowDetailsModal(false)}>
          <Card className="w-full max-w-5xl max-h-[92vh] overflow-auto" onClick={e => e.stopPropagation()}>
            ...
            <CardContent className="space-y-4 text-sm">
              <section className="border rounded-lg p-4"><h3 className="font-semibold mb-2">Product Information</h3><div className="grid gap-3 sm:grid-cols-[100px_1fr_1fr]"><div>{selectedInquiry.productPhoto ? <img src={selectedInquiry.productPhoto} className="h-20 w-20 rounded border object-cover" /> : <div className="h-20 w-20 rounded border bg-muted" />}</div><div><div className="text-muted-foreground">Product name</div><div>{selectedInquiry.productName}</div></div><div><div className="text-muted-foreground">Variant handling</div><div>{modeLabel(selectedInquiry)}</div></div></div></section>
              ...
              <section className="border rounded-lg p-4"><h3 className="font-semibold mb-2">Inquiry lines</h3><div className="overflow-x-auto border rounded"><table className="min-w-full text-xs"><thead className="bg-muted/30"><tr><th className="p-2 text-left">Variant</th><th className="p-2 text-left">Color</th><th className="p-2 text-right">Qty</th><th className="p-2 text-right">RMB/pc</th><th className="p-2 text-right">INR/pc</th></tr></thead><tbody>{inquiryLinesForDisplay(selectedInquiry).map(line => <tr key={line.id} className="border-t"><td className="p-2">{line.variant || '—'}</td><td className="p-2">{line.color || '—'}</td><td className="p-2 text-right">{line.quantity || 0}</td><td className="p-2 text-right">{line.rmbPricePerPiece || 0}</td><td className="p-2 text-right">{line.inrPricePerPiece || 0}</td></tr>)}</tbody></table></div></section>
