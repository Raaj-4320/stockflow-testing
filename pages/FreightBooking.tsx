                        ...
                        <tbody>
                          ...
                          {lineItems.map(line => (
                            <tr key={line.id} className="border-t">
                              ...
                              <td className="p-1">{form.source === 'new' && <Button type="button" size="sm" variant="outline" onClick={() => removeLine(line.id)}>Remove</Button>}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
        ...
        <div className="fixed inset-0 z-[65] bg-black/45 flex items-center justify-center p-4" onClick={() => setShowSummaryModal(false)}>
          <Card className="w-full max-w-4xl" onClick={e => e.stopPropagation()}>
            ...
            <CardContent className="space-y-4 text-sm">
              <div className="grid gap-3 sm:grid-cols-[100px_1fr]"><div>{summaryInquiry.productPhoto ? <img src={summaryInquiry.productPhoto} className="h-24 w-24 rounded border object-cover" /> : <div className="h-24 w-24 rounded border bg-muted" />}</div><div className="grid gap-1"><div><span className="text-muted-foreground">Product:</span> {summaryInquiry.productName}</div><div><span className="text-muted-foreground">Inquiry ID:</span> {summaryInquiry.id}</div><div><span className="text-muted-foreground">Variant handling:</span> {modeLabel(summaryInquiry)}</div><div><span className="text-muted-foreground">Status:</span> {summaryInquiry.status}</div></div></div>
