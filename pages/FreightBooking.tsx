...
export default function FreightBooking() {
  ...
  const applySelectedVariants = () => {
    ...
    if (form.quantityMode === 'order_level' && ids.length === 1) {
      const onlyId = ids[0];
              ...
              <section className="border rounded-lg p-4 space-y-3">
                ...
                {form.variantSelectionMode === 'exact' && (
                  <>
                    ...
                    {!!appliedLines.length && (
                      <section className="border rounded-lg p-3 space-y-2">
                        <h4 className="font-medium">Calculation rows (selected variants)</h4>
                        <div className="overflow-x-auto border rounded">
                            ...
                            <tbody>
                              {appliedLines.map(line => (
                                <tr key={line.id} className="border-t">
                                  ...
                                  <td className="p-1"><Input type="number" min="0" value={line.totalPieces} disabled={form.quantityMode === 'line_level' || (form.quantityMode === 'order_level' && appliedLines.length === 1)} onChange={e => updateLine(line.id, { totalPieces: Number(e.target.value) || 0 })} /></td>
