...
export default function PurchasePanel() {
  ...
  const activeLines = useMemo(() => {
    if (sourceMode === 'new') {
      ...
      return [pricingEntries[key] || {
        key,
        label: `${newProductDraft.name || 'New Product'} / Default`,
        stock: 0,
        quantity: '',
        unitCost: '',
        variant: newProductDraft.variants[0],
        color: newProductDraft.colors[0],
      }];
    }
  ...
  }, [sourceMode, selectedVariants, pricingEntries, newProductDraft]);
  ...
  const goToPricing = () => {
    ...
    if (sourceMode === 'new') {
      seed['new-default'] = pricingEntries['new-default'] || {
        key: 'new-default',
        label: `${newProductDraft.name || 'New Product'} / Default`,
        stock: 0,
        quantity: '',
  ...
  const editOrder = (order: PurchaseOrder) => {
    ...
    if (first.sourceType === 'inventory' && first.productId) {
      ...
      setNewProductDraft({
        ...
        variants: first.pendingProductDraft?.variants || [],
        colors: first.pendingProductDraft?.colors || [],
        sellPrice: first.pendingProductDraft?.sellPrice ?? '',
      });
      setPricingEntries({
        'new-default': {
          key: 'new-default',
          label: `${first.productName || 'New Product'} / Default`,
          stock: 0,
        ...
        {wizardStep === 'newProduct' && (
          <div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="md:col-span-2"><Label>Image URL (optional)</Label><Input value={newProductDraft.image} onChange={e => updateNewDraft({ image: e.target.value })} /></div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      {sourceMode === 'new' && (
                        <>
                          <div>
                            <Label>Variant</Label>
                            <select className="h-10 w-full rounded-md border px-3 text-sm" value={line.variant || ''} onChange={e => setPricingEntries(prev => ({ ...prev, [line.key]: { ...line, variant: e.target.value || undefined } }))}>
                              <option value="">Default</option>
                              {newProductDraft.variants.map(v => <option key={v} value={v}>{v}</option>)}
                            </select>
                          </div>
                          <div>
                            <Label>Color</Label>
                            <select className="h-10 w-full rounded-md border px-3 text-sm" value={line.color || ''} onChange={e => setPricingEntries(prev => ({ ...prev, [line.key]: { ...line, color: e.target.value || undefined } }))}>
                              <option value="">Default</option>
                              {newProductDraft.colors.map(c => <option key={c} value={c}>{c}</option>)}
                            </select>
                          </div>
                        </>
                      )}
