              ...
              {parties.map(p => (
                <div key={p.id} className="rounded-xl border p-3">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                    ...
                    <SummaryCard label="Remaining Payable" value={`₹${formatNumber(partyLedgers.get(p.id)?.summary.remainingPayable || 0)}`} />
                  </div>
                  <div className="mt-2 grid grid-cols-2 gap-2 text-xs"><SummaryCard label="Net Payable" value={`₹${formatNumber(partyLedgers.get(p.id)?.summary.netPayable || 0)}`} /></div>
                  <div className="mt-2">
                  ...
                  <table className="w-full text-xs">
                    <thead className="bg-slate-50">
                      <tr>
                        ...
                        <th className="p-2 text-right">Purchase / Payable +</th><th className="p-2 text-right">Actual Payment</th><th className="p-2 text-right">Payable Applied</th><th className="p-2 text-right">Credit Created</th><th className="p-2 text-right">Credit Used</th><th className="p-2 text-right">Net Payable</th>
                      </tr>
                    </thead>
                    <tbody>
                      {partyLedgerRows.map((row, idx) => (
                        <tr key={`${row.reference}-${idx}`} className="border-t">
                          ...
                          <td className="p-2 text-right font-semibold">₹{formatNumber(row.netPayable || 0)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
