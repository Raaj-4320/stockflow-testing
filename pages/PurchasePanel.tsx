...
export default function PurchasePanel() {
  ...
  const saveOrder = async () => {
    ...
    if (import.meta.env.DEV) {
      console.info('[PURCHASE_CREDIT_APPLY_TRACE]', {
        partyId: party.id,
        partyName: party.name,
        grandTotal: Number((taxableAmount + gstAmount).toFixed(2)),
        initialPaid,
        latestAvailablePartyCredit: Number(latestAvailablePartyCredit.toFixed(2)),
        latestMaxCreditUsable: Number(latestMaxCreditUsable.toFixed(2)),
        uiPartyCreditToApply,
        partyCreditTouched,
        finalCreditToApply: Number(finalCreditToApply.toFixed(2)),
        orderId: order.id,
        applyCalled: finalCreditToApply > 0,
        applyResult,
        orderHasPartyCreditPaymentHistory,
      });
    }
    if (!creditApplyFailed) {
