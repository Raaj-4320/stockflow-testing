...
export default function PurchasePanel() {
  ...
  const partyLedgerRows = useMemo(() => {
    if (!expandedPartyId) return [] as any[];
    const party = parties.find((p) => p.id === expandedPartyId);
    if (!party) return [] as any[];
    const isPartyMatch = (pid?: string, pname?: string) => (pid && pid === party.id) || (!!pname && normalizePartyName(pname) === normalizePartyName(party.name));
    const rows: any[] = [];
    (orders || []).filter((o) => isPartyMatch(o.partyId, o.partyName)).forEach((order) => {
      rows.push({ date: order.orderDate || order.createdAt, type: 'Purchase Created', reference: order.billNumber || order.id, description: `PO ${order.id}`, payable: Math.max(0, Number(order.totalAmount || 0)), payment: 0, creditAdded: 0, creditUsed: 0 });
      (order.paymentHistory || []).forEach((p: any) => {
        const method = String(p.method || '').toLowerCase();
        rows.push({ date: p.paidAt || order.updatedAt, type: method === 'party_credit' ? 'Credit Used' : 'Order Payment', reference: p.id || order.id, description: p.note || method || 'Payment', payable: 0, payment: method === 'party_credit' ? 0 : Math.max(0, Number(p.amount || 0)), creditAdded: 0, creditUsed: method === 'party_credit' ? Math.max(0, Number(p.amount || 0)) : 0 });
      });
    });
    (supplierPayments || []).filter((p: any) => !p.deletedAt && isPartyMatch(p.partyId, p.partyName)).forEach((p: any) => {
      rows.push({ date: p.paidAt || p.createdAt, type: 'Payment Given', reference: p.voucherNo || p.id, description: p.note || p.method || 'Party payment', payable: 0, payment: Math.max(0, Number(p.paymentAppliedToPayable ?? p.amount ?? 0)), creditAdded: Math.max(0, Number(p.partyCreditCreated || 0)), creditUsed: 0 });
    });
    const sorted = rows.sort((a, b) => new Date(a.date || 0).getTime() - new Date(b.date || 0).getTime());
    let runningPayable = 0;
    return sorted.map((row) => {
      runningPayable = Math.max(0, Number((runningPayable + row.payable - row.payment - row.creditUsed).toFixed(2)));
              ...
              {parties.map(p => (
                <div key={p.id} className="rounded-xl border p-3">
                  ...
                  <div className="mt-2 grid grid-cols-3 gap-2 text-xs">
                    ...
                    <SummaryCard label="Payable" value={`₹${formatNumber(partyFinancials.get(p.id)?.remaining || 0)}`} />
                  </div>
                  ...
                  <table className="w-full text-xs">
                    <thead className="bg-slate-50">
                      <tr>
                        ...
                        <th className="p-2 text-right">Payable +</th><th className="p-2 text-right">Payment -</th><th className="p-2 text-right">Credit +</th><th className="p-2 text-right">Credit Used -</th><th className="p-2 text-right">Balance / Net Payable</th>
                      </tr>
                    </thead>
                    <tbody>
                      {partyLedgerRows.map((row, idx) => (
                        <tr key={`${row.reference}-${idx}`} className="border-t">
                          ...
                          <td className="p-2 text-right">{row.payable ? `₹${formatNumber(row.payable)}` : '—'}</td>
                          <td className="p-2 text-right">{row.payment ? `₹${formatNumber(row.payment)}` : '—'}</td>
                          <td className="p-2 text-right">{row.creditAdded ? `₹${formatNumber(row.creditAdded)}` : '—'}</td>
                          <td className="p-2 text-right">{row.creditUsed ? `₹${formatNumber(row.creditUsed)}` : '—'}</td>
                          <td className="p-2 text-right font-semibold">₹{formatNumber(row.runningPayable || 0)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
