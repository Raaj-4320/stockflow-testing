...
export default function Cashbook() {
  ...
  const safeCashAdjustments = Array.isArray(data.cashAdjustments) ? data.cashAdjustments : [];
  const safeDeleteCompensations = Array.isArray(data.deleteCompensations) ? data.deleteCompensations : [];
  const safeUpdatedTransactionEvents = Array.isArray(data.updatedTransactionEvents) ? data.updatedTransactionEvents : [];
  const safeCustomers = Array.isArray(data.customers) ? data.customers : [];
  ...
  const rows = useMemo(() => {
    ...
    const purchaseRows: Row[] = safePurchaseOrders.flatMap((po: PurchaseOrder) => {
      ...
      const pays = (Array.isArray(po.paymentHistory) ? po.paymentHistory : []).map((p) => ({ id: `pop-${po.id}-${p.id}`, date: p.paidAt, type: 'payment' as LedgerType, description: `Supplier Payment #${p.id.slice(-6)} — ${po.partyName}`,
  ...
  const kpi = useMemo(() => {
    ...
    const receivable = getCanonicalCustomerBalanceSnapshot(safeCustomers, safeTransactions).reduce((s, c) => s + Math.max(0, (c.totalDue || 0) - (c.storeCredit || 0)), 0);
    const payable = safePurchaseOrders.reduce((s, p) => s + Math.max(0, Number(p.remainingAmount ?? Math.max(0, (p.totalAmount || 0) - (p.totalPaid || 0)))), 0);
