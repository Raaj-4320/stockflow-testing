...
export default function Cashbook() {
  ...
  }, [safeTransactions, safePurchaseOrders, safeSupplierPayments, safeExpenses, safeCashAdjustments]);
          ...
          <div className="overflow-auto">
            <table className="min-w-[2600px] w-full text-xs">
              <thead className="sticky top-0 bg-slate-50"><tr className="text-left border-b">
                <th>DATE</th><th>Customer Name</th><th>Bill Ref</th><th>Invoice Number</th><th>CREDIT A/C</th><th>Payment Type</th><th>Details</th><th>Avai. Qty</th><th>Selling Qty</th><th>Selling Price</th><th>Bill Total</th><th>Total</th><th>Balance INR</th><th>Credit Amount</th><th>Buying Price</th><th>Total Buying Price</th><th>Profit</th><th>Column1</th><th>Column2</th><th>Column3</th>
              </tr></thead>
              <tbody>
                {registerRows.map((r) => <tr key={r.id} className="border-b">
                  <td>{new Date(r.date).toLocaleString()}</td><td>{r.customerName || '—'}</td><td>{r.billRef || '—'}</td><td>{r.invoiceNumber || '—'}</td><td>{r.creditAc || 'XXX'}</td><td>{r.paymentType || '—'}</td><td>{r.details || '—'}</td><td>{r.avaiQty || '—'}</td><td>{r.sellingQty || '—'}</td><td>{r.sellingPrice || '—'}</td><td>{r.billTotal || '—'}</td><td>{r.total || '—'}</td><td>{r.balanceInr || '—'}</td><td>{r.creditAmount || '—'}</td><td>{r.buyingPrice || '—'}</td><td>{r.totalBuyingPrice || '—'}</td><td>{r.profit || '—'}</td><td>{r.column1 || ''}</td><td>{r.column2 || ''}</td><td>{r.column3 || ''}</td>
                </tr>)}
              </tbody>
            </table>
          </div>
