...
export default function Cashbook() {
  ...
  const [full, setFull] = useState(false); const [visibleRowCount, setVisibleRowCount] = useState(35);
  ...
  const kpi = useMemo(() => {
    ...
    const receivable = canonicalSnapshot.reduce((s, c) => s + Math.max(0, toNum(c.totalDue) - toNum(c.storeCredit)), 0);
    const payable = safePurchaseOrders.reduce((s, p) => s + Math.max(0, toNum(p.remainingAmount ?? Math.max(0, toNum(p.totalAmount) - toNum(p.totalPaid)))), 0);
    return { cash, bank, receivable, payable };
  }, [rows, safeCustomers, safeTransactions, safePurchaseOrders]);

  useEffect(() => setVisibleRowCount(35), [from, to, payFilter, typeFilter, search, sort]);
