            ...
            <div className="space-y-3">
              <div className="space-y-2">
                {returnTransactions.map((tx) => {
                  ...
                  return (
                    <div key={tx.id} className="rounded-xl border bg-card p-3 grid gap-2 md:grid-cols-[130px_180px_minmax(0,1fr)_92px_120px_132px] items-center">
        ...
        <div className="fixed inset-0 bg-black/70 z-[90] flex items-center justify-center p-4" onClick={() => setIsReturnPopupOpen(false)}>
          <Card className="w-full max-w-5xl max-h-[90vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <CardContent className="p-3 space-y-3 overflow-y-auto max-h-[calc(90vh-138px)]">
              <div className="grid gap-1.5 md:grid-cols-2 text-xs rounded-lg border p-2.5 bg-muted/10">
                ...
                <div><span className="text-muted-foreground">Bill:</span> <span className="font-semibold">#{selectedReturnTx.id}</span></div>
                ...
                <div className="md:col-span-2"><span className="text-muted-foreground">Settlement:</span> <span className="font-semibold">Cash ₹{formatMoneyPrecise(Number(selectedSettlement.cashPaid || 0))} • Online ₹{formatMoneyPrecise(Number(selectedSettlement.onlinePaid || 0))} • Credit ₹{formatMoneyPrecise(Number(selectedSettlement.creditDue || 0))}</span></div>
              </div>

              <div className="space-y-1.5">
                <div className="text-xs font-semibold text-muted-foreground px-1">Products in this transaction</div>
                {selectedReturnLines.map((line) => {
                  ...
                  <div key={line.key} className="rounded-md border px-2 py-1.5">
                    <div className="grid grid-cols-[30px_minmax(0,1fr)_72px_58px_64px_74px] md:grid-cols-[34px_minmax(0,1fr)_90px_76px_74px_88px] items-center gap-1.5 md:gap-2 text-xs">
                      ...
                      <div className="min-w-0 truncate font-medium">
                        ...
                        {variantParts && <span className="text-muted-foreground"> ({variantParts})</span>}
                      </div>
              ...
              <div className="rounded-md border p-2.5 bg-orange-50/40 space-y-1.5 text-xs">
                <div className="font-semibold">Return Preview Summary</div>
