...
export default function Sales() {
  ...
  const shouldStoreCreditTrace = (customer?: { name?: string; phone?: string; id?: string } | null, storeCreditCreatedValue?: number) => {
    const name = String(customer?.name || '').toLowerCase();
    const phone = String(customer?.phone || '').trim();
    return Number(storeCreditCreatedValue || 0) > 0 || name.includes('ruchit') || phone === '9726759110';
  };
  ...
  const completeCheckout = () => {
      ...
      let checkoutStoreCreditCreated = !isReturnMode && storeOverpaymentAsCredit && finalCustomer ? Math.max(0, rawOverpaymentValue) : 0;
      if (!isReturnMode && cashPaid > 0) {
          ...
          checkoutStoreCreditCreated = storeCreditCreated;
      ...
      if (shouldStoreCreditTrace(finalCustomer, tx.storeCreditCreated)) {
        console.log("[STORE CREDIT TRACE]", JSON.stringify({
          stage: "pos_before_processTransaction",
          txId: tx.id,
          customerId: finalCustomer?.id || null,
          customerName: finalCustomer?.name || null,
          total: tx.total,
          payableAfterStoreCredit: payableAfterCreditWhole,
          cashPaidInput,
          onlinePaidInput,
          creditDueInput,
          rawOverpaymentValue,
          storeOverpaymentAsCredit,
          shouldAllowOverpayAsCreditSubmit,
          storeCreditCreated: tx.storeCreditCreated || 0,
          storeCreditUsed: tx.storeCreditUsed || 0,
          saleSettlement: tx.saleSettlement || null
        }, null, 2));
      }
