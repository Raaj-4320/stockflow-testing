    ...
    <div className={`h-full rounded-xl border p-3 md:p-4 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_390px] gap-3 ${isReturnMode ? 'bg-orange-50/20 border-orange-200' : 'bg-background border-border'}`}>
        ...
        <div className="bg-card border rounded-xl p-3 space-y-3">
          <div className={`grid gap-3 items-center ${isReturnMode ? 'md:grid-cols-[minmax(0,1fr)_420px]' : 'md:grid-cols-[minmax(0,1fr)_220px]'}`}>
            ...
            <div className={`grid gap-2 ${isReturnMode ? 'grid-cols-4' : 'grid-cols-2'}`}>
              ...
              {isReturnMode && (
                <>
                  <select className="h-9 rounded-md border border-input bg-background px-3 text-sm" value={returnDateFilter} onChange={e => setReturnDateFilter(e.target.value as 'all' | '30d' | '90d')}>
                    <option value="90d">Last 90 days</option>
                    <option value="30d">Last 30 days</option>
                    <option value="all">All dates</option>
                  </select>
                  <select className="h-9 rounded-md border border-input bg-background px-3 text-sm" value={returnSort} onChange={e => setReturnSort(e.target.value as 'newest' | 'oldest' | 'amount_high' | 'amount_low')}>
            ...
            <div className="space-y-3">
              <div className="space-y-2">
                {returnTransactions.map((tx) => {
                  ...
                  const itemLabel = (tx.items || []).length <= 1 ? (tx.items?.[0]?.name || '—') : `${(tx.items || []).length} items`;
                  return (
                    <div key={tx.id} className="rounded-xl border bg-card p-3 grid gap-2 md:grid-cols-[140px_minmax(0,1fr)_92px_110px_132px_130px] items-center">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground truncate">Bill #{tx.id} • {itemLabel}</div>
                      </div>
                      ...
                      <div className="text-xs text-muted-foreground">Cash ₹{formatMoneyPrecise(tx.saleSettlement?.cashPaid || 0)} • Online ₹{formatMoneyPrecise(tx.saleSettlement?.onlinePaid || 0)}</div>
      ...
      {isReturnMode && isReturnPopupOpen && selectedReturnTx && (
        <div className="fixed inset-0 bg-black/70 z-[90] flex items-center justify-center p-4" onClick={() => setIsReturnPopupOpen(false)}>
          <Card className="w-full max-w-6xl max-h-[92vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <CardHeader className="border-b py-3">
              <CardTitle className="text-base">Create Return — Bill #{selectedReturnTx.id}</CardTitle>
            </CardHeader>
            <CardContent className="p-4 space-y-4 overflow-y-auto max-h-[calc(92vh-140px)]">
              <div className="grid gap-2 md:grid-cols-3 text-xs rounded-lg border p-3 bg-muted/10">
                <div><span className="text-muted-foreground">Date:</span> <span className="font-semibold">{new Date(selectedReturnTx.date).toLocaleString()}</span></div>
                <div><span className="text-muted-foreground">Customer:</span> <span className="font-semibold">{selectedReturnTx.customerName || 'Walk-in customer'}</span></div>
                <div><span className="text-muted-foreground">Phone:</span> <span className="font-semibold">{selectedReturnCustomer?.phone || '—'}</span></div>
                <div><span className="text-muted-foreground">Current Due:</span> <span className="font-semibold">₹{formatMoneyPrecise(dueBefore)}</span></div>
                ...
                <div className="md:col-span-3"><span className="text-muted-foreground">Settlement:</span> <span className="font-semibold">Cash ₹{formatMoneyPrecise(Number(selectedSettlement.cashPaid || 0))} • Online ₹{formatMoneyPrecise(Number(selectedSettlement.onlinePaid || 0))} • Credit Due ₹{formatMoneyPrecise(Number(selectedSettlement.creditDue || 0))}</span></div>
              </div>

              <div className="space-y-2">
                {selectedReturnLines.map((line) => (
                  <div key={line.key} className="rounded-lg border p-3">
                    <div className="grid gap-3 md:grid-cols-[56px_minmax(0,1fr)_170px_180px] items-center">
                      <div className="h-14 w-14 rounded border bg-muted overflow-hidden">{line.image ? <img src={line.image} alt={line.name} className="h-full w-full object-contain" /> : <Package className="w-full h-full p-3 opacity-30" />}</div>
                      <div>
                        <div className="font-semibold text-sm">{line.name}</div>
                        <div className="text-xs text-muted-foreground">Variant: {line.variant === NO_VARIANT ? 'Default' : line.variant} • Color: {line.color === NO_COLOR ? 'Default' : line.color}</div>
                        <div className="text-xs text-muted-foreground">Sold Qty {line.originalQty} • Returned {line.returnedQty} • Returnable {line.returnableQty}</div>
                      </div>
                      <div className="text-xs">
                        <div className="text-muted-foreground">Sold Price</div>
                        <div className="font-semibold text-sm">₹{formatMoneyPrecise(line.unitPrice)}</div>
                      </div>
                      <div className="grid grid-cols-[78px_minmax(0,1fr)] gap-2 items-center">
                        <Input
                          type="number"
                          min="0"
                        ...
                        />
                        <div className="font-semibold text-sm text-right">₹{formatMoneyPrecise(line.selectedSubtotal)}</div>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="rounded-lg border p-3 bg-orange-50/40 space-y-2 text-xs">
                <div className="font-semibold text-sm">Return Summary Preview</div>
                <div className="grid md:grid-cols-2 gap-2">
                  <div className="rounded border bg-white p-2 flex justify-between"><span>Return Subtotal</span><span className="font-semibold">₹{formatMoneyPrecise(returnSubtotal)}</span></div>
                  <div className="rounded border bg-white p-2 flex justify-between"><span>Return Total</span><span className="font-semibold">₹{formatMoneyPrecise(returnSubtotal)}</span></div>
                  ...
                  <div className="rounded border bg-white p-2 flex justify-between"><span>Credit Before → After</span><span className="font-semibold">₹{formatMoneyPrecise(creditBefore)} → ₹{formatMoneyPrecise(creditAfter)}</span></div>
                ...
                {dominantMode === null && returnSubtotal > 0 && (
                  <p className="text-destructive font-medium">Mixed financial outcome detected. Confirm is locked for safety until a single handling path applies.</p>
                )}
                {returnSubmitError && <p className="text-destructive font-medium">{returnSubmitError}</p>}
              </div>
            </CardContent>
