...
export default function Sales() {
  ...
  const completeCheckout = () => {
      ...
      const creditDue = isReturnMode ? 0 : clampCreditDueAmount(Math.max(0, payableAfterCreditWhole - cashPaid - onlinePaid));
      const settlementCashPaid = !isReturnMode
        ? Math.min(cashPaid, Math.max(0, payableAfterCreditWhole - onlineAppliedToSale - creditDue))
        : 0;
  ...
  const cashAppliedToSaleValue = Math.min(cashPaidValue, Math.max(0, payableAfterStoreCredit - onlineAppliedValue - autoCreditDueValue));
  const splitTotalValue = roundMoneyWhole(cashAppliedToSaleValue + onlineAppliedValue + autoCreditDueValue);
  const amountAppliedToSaleValue = roundMoneyWhole(storeCreditUsed + cashAppliedToSaleValue + onlineAppliedValue);
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            ...
            <div className="space-y-2 rounded-lg border p-2.5 bg-muted/10">
              ...
              <div className="text-xs space-y-1 border-t pt-2">
                <div className="flex justify-between"><span>Paid Applied (Cash + Online)</span><span>₹{formatMoneyWhole(cashAppliedToSaleValue + onlineAppliedValue)}</span></div>
                <div className="flex justify-between"><span>Online Paid</span><span>₹{formatMoneyWhole(onlineAppliedValue)}</span></div>
            ...
            <Button className="w-full h-12 text-base font-bold" onClick={completeCheckout} disabled={transactionSyncStatus.phase === 'pending' || transactionSyncStatus.phase === 'committing'}>
              {transactionSyncStatus.phase === 'pending' || transactionSyncStatus.phase === 'committing' ? 'Processing…' : (
                <span className="flex flex-col items-center leading-tight">
                  <span>Confirm & Pay ₹{formatMoneyWhole(amountAppliedToSaleValue)}</span>
              ...
              <div className="border rounded-xl p-4 space-y-4 overflow-y-auto">
                {!isReturnMode && (
                  <div className="space-y-2.5 rounded-lg border p-3 bg-muted/10">
                    <p className="text-xs font-bold uppercase text-muted-foreground">Settlement Split</p>
                    <div className="space-y-1.5">
                      <Label className="text-[12px] font-bold uppercase text-muted-foreground">Total Amount</Label>
