...
export default function Sales() {
  ...
  const completeCheckout = () => {
      ...
      console.log("[STORE CREDIT DEBUG]", JSON.stringify({
        stage: "pos_submit_payload",
        transactionType: tx?.type || (isReturnMode ? "return" : "sale"),
        customerId: finalCustomer?.id || null,
        customerName: finalCustomer?.name || null,
        originalInvoiceTotal,
        payableAfterStoreCredit: payableAfterCredit,
        cashPaid,
        onlinePaid,
        creditDue,
        rawOverpaymentValue,
        ...
        storeCreditCreated: !isReturnMode && storeOverpaymentAsCredit && finalCustomer ? Math.max(0, rawOverpaymentValue) : 0,
        payloadStoreCreditCreated: tx.storeCreditCreated,
        payloadStoreCreditUsed: tx.storeCreditUsed,
        saleSettlement: tx.saleSettlement
      }, null, 2));
  ...
  const handleToggleStoreCredit = () => {
    ...
    if (!enabling) {
      ...
      console.log("[STORE CREDIT DEBUG]", JSON.stringify({
        stage: "store_credit_ui_action",
        action: "remove",
        customerId: resolvedSelectedCustomer?.id || null,
        availableStoreCredit,
        appliedStoreCredit,
        remainingStoreCreditAfterInvoice,
    ...
    console.log("[STORE CREDIT DEBUG]", JSON.stringify({
      ...
      customerId: resolvedSelectedCustomer?.id || null,
      availableStoreCredit,
      appliedStoreCredit: maxCredit,
      remainingStoreCreditAfterInvoice: Math.max(0, availableStoreCredit - maxCredit),
      payableAfterStoreCredit
    }, null, 2));
  };
              ...
              <div className="rounded-lg border p-3 space-y-2 bg-muted/10">
                ...
                {useStoreCreditApplied && (
                  <div className="space-y-1">
                    ...
                    <Input type="number" min="0" step="0.01" value={storeCreditInput} onChange={(e) => { setStoreCreditInput(e.target.value); console.log("[STORE CREDIT DEBUG]", JSON.stringify({ stage: "store_credit_ui_action", action: "manual_change", customerId: resolvedSelectedCustomer?.id || null, availableStoreCredit, appliedStoreCredit: Number(e.target.value || 0), remainingStoreCreditAfterInvoice: Math.max(0, availableStoreCredit - Number(e.target.value || 0)), payableAfterStoreCredit }, null, 2)); }} />
                  </div>
                )}
                <div className="text-xs space-y-1 border-t pt-2">
