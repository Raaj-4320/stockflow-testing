...
export default function Transactions() {
  ...
  useEffect(() => {
    const allowedIds = new Set(filteredTransactions.map(tx => tx.id));
    setSelectedTransactionIds(prev => prev.filter(id => allowedIds.has(id)));
  }, [filteredTransactions]);

  const isAllFilteredSelected = filteredTransactions.length > 0 && filteredTransactions.every(tx => selectedTransactionIds.includes(tx.id));
  const selectedTransactions = useMemo(
    () => filteredTransactions.filter(tx => selectedTransactionIds.includes(tx.id)),
  ...
  const toggleSelectAllFiltered = () => {
    if (isAllFilteredSelected) {
      setSelectedTransactionIds(prev => prev.filter(id => !filteredTransactions.some(tx => tx.id === id)));
      return;
    }
  ...
  const handleBatchEdit = () => {
    ...
    setEditingTx(target);
    setEditingAmount(String(Math.abs(target.total)));
  };
    ...
    <div className="space-y-6 pb-20 md:pb-0">
      ...
      {selectedTransactionIds.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 rounded-lg border bg-muted/20 p-3">
          <Badge variant="secondary" className="h-8 px-3">{selectedTransactionIds.length} selected</Badge>
          <Button
            variant="outline"
            className="h-8"
            disabled={selectedTransactionIds.length !== 1}
            onClick={handleBatchEdit}
            title={selectedTransactionIds.length !== 1 ? 'Select exactly one transaction to edit.' : 'Edit selected transaction'}
          >
            <Edit className="w-3.5 h-3.5 mr-1.5" /> Edit Selected
          </Button>
          <Button variant="destructive" className="h-8" onClick={handleBatchDelete}>
            <X className="w-3.5 h-3.5 mr-1.5" /> Delete Selected
          </Button>
          <Button variant="ghost" className="h-8" onClick={() => setSelectedTransactionIds([])}>
            Clear
          </Button>
        </div>
      )}
