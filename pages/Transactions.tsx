...
export default function Transactions() {
  ...
  const editRiskBanner = useMemo(() => {
    ...
    if (editingTx.type === 'sale' && editingSaleLinkageInfo?.hasLinkedReturns) {
      return { tone: 'bg-red-50 border-red-200 text-red-800', title: 'High-risk linked-return edit', detail: 'Some line edits are restricted to preserve source-linked return audit truth.' };
    }
    if (editingTx.type === 'sale' && (editingSaleLinkageInfo?.linkedPaymentsCount || 0) > 0) {
      return { tone: 'bg-amber-50 border-amber-200 text-amber-800', title: 'Linked payments exist', detail: 'This edit will recalculate due / collections via reconcile before save.' };
    }
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <Card className="w-full max-w-5xl max-h-[90vh] overflow-hidden">
            ...
            <CardContent className="p-3 space-y-3 overflow-y-auto max-h-[calc(90vh-136px)]">
              ...
              {editingSectionWarning?.section === 'general' && <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{editingSectionWarning.message}</div>}
              ...
              {editingTx.type === 'sale' && (editingSaleLinkageInfo?.hasLinkedReturns || editingSaleLinkageInfo?.linkedPaymentsCount) && (
                <div className="rounded-md border p-2.5 bg-amber-50/60 text-[13px] space-y-1">
                  <div className="font-semibold">Impact warning</div>
                  {editingSaleLinkageInfo.hasLinkedReturns && <div>This sale has linked returns. Returned lines cannot be removed, dropped below returned qty, or repriced.</div>}
                  {editingSaleLinkageInfo.linkedPaymentsCount > 0 && <div>This sale has {editingSaleLinkageInfo.linkedPaymentsCount} payment collection(s) on same-customer timeline. Save will recalculate due/store-credit through reconcile.</div>}
                </div>
              )}
                ...
                <div className="space-y-2">
                  ...
                  {editingTx.type === 'return' && editingReturnPreview && (
                    <div className="rounded-md border p-2.5 bg-orange-50/40 space-y-1.5 text-[13px]">
                      ...
                      <div className="rounded border bg-white p-2 flex justify-between"><span>Cash Refund</span><span className="font-semibold">₹{formatMoneyPrecise(editingReturnPreview.cashRefund)}</span></div>
                      <div className="rounded border bg-white p-2 flex justify-between"><span>Online Refund</span><span className="font-semibold">₹{formatMoneyPrecise(editingReturnPreview.onlineRefund)}</span></div>
                    </div>
                  )}
                  {editingTx.type === 'payment' && (
                    <div className="rounded-md border p-2.5 space-y-1.5 text-[13px]">
