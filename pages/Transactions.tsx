                        ...
                        <tr key={record.id} className="hover:bg-muted/30 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <div className="text-xs">
                              <div>{record.deletedBy || 'unknown'}</div>
                              <div className="text-muted-foreground">{record.deletedByRole || 'unknown'}</div>
                            </div>
                          </td>
          ...
          <Card className="w-full max-w-2xl animate-in zoom-in duration-200 flex flex-col max-h-[90vh] shadow-2xl">
            ...
            <CardContent className="overflow-auto space-y-4 p-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                ...
                <div><span className="text-muted-foreground">Deleted by:</span> {selectedDeletedTx.deletedBy || 'unknown'} ({selectedDeletedTx.deletedByRole || 'unknown'})</div>
                ...
                <div><span className="text-muted-foreground">Payment:</span> {selectedDeletedTx.paymentMethod || 'N/A'}</div>
                <div><span className="text-muted-foreground">Amount:</span> ₹{Math.abs(selectedDeletedTx.amount || 0).toLocaleString()}</div>
              </div>

              <div className="rounded-lg border p-3 bg-muted/10">
                <p className="font-semibold mb-2">Impact Snapshot</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                  <div>
                    <p className="font-semibold text-muted-foreground mb-1">Before delete</p>
                    <p>Customer due: ₹{selectedDeletedTx.beforeImpact.customerDue.toLocaleString()}</p>
                    <p>Store credit: ₹{selectedDeletedTx.beforeImpact.customerStoreCredit.toLocaleString()}</p>
                    <p>Active tx count: {selectedDeletedTx.beforeImpact.activeTransactionsCount}</p>
                    <p>Cash estimate: ₹{selectedDeletedTx.beforeImpact.estimatedCashFromActiveTransactions.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="font-semibold text-muted-foreground mb-1">After delete</p>
                    <p>Customer due: ₹{selectedDeletedTx.afterImpact.customerDue.toLocaleString()}</p>
                    <p>Store credit: ₹{selectedDeletedTx.afterImpact.customerStoreCredit.toLocaleString()}</p>
                    <p>Active tx count: {selectedDeletedTx.afterImpact.activeTransactionsCount}</p>
                    <p>Cash estimate: ₹{selectedDeletedTx.afterImpact.estimatedCashFromActiveTransactions.toLocaleString()}</p>
                  </div>
                </div>
              </div>

              <div className="rounded-lg border p-3">
                <p className="font-semibold mb-2">Original transaction snapshot</p>
                <pre className="text-xs whitespace-pre-wrap break-words">{JSON.stringify(selectedDeletedTx.originalTransaction, null, 2)}</pre>
              </div>
            </CardContent>
          </Card>
        </div>
