...
export default function Transactions() {
  ...
  const openTransactionEditor = async (tx: Transaction) => {
    if (!can(roleSession, 'editTransaction')) {
  ...
  const handleBatchDeleteTransactions = async () => {
    ...
    if (!can(roleSession, 'deleteTransaction')) {
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0">
        ...
        <div className="flex flex-wrap items-center gap-2 bg-muted/30 p-2 rounded-lg border w-full md:w-auto">
            ...
            {selectedTransactionIds.length > 0 && (
              <>
                ...
                <Button variant="destructive" onClick={() => void handleBatchDeleteTransactions()} className="h-9 text-sm">Batch Delete</Button>
              </>
            )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4">
          ...
          {can(roleSession, 'viewProfitAnalytics') && (
                                    ...
                                    <tr key={tx.id} className="hover:bg-muted/30 transition-colors group">
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            <div className="flex items-center justify-center gap-1">
                                                ...
                                                {!tx.id.startsWith('upfront-') && !isSupplierPaymentVirtualTransaction(tx) && <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => void openTransactionEditor(tx)}><Edit className="w-3.5 h-3.5" /></Button>}
                                                {!tx.id.startsWith('upfront-') && !isSupplierPaymentVirtualTransaction(tx) && <Button variant="ghost" size="icon" className="h-7 w-7 text-red-600" onClick={() => void openDeleteModal(tx)}><X className="w-3.5 h-3.5" /></Button>}
