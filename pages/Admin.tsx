                      ...
                      <table className="w-full text-xs">
                        <thead className="sticky top-0 bg-muted/50">
                          <tr className="text-left">
                            ...
                            <th className="p-2 text-right">Prev Stock</th>
                            <th className="p-2 text-right">Prev Buy</th>
                            <th className="p-2 text-right">Next Buy</th>
                          </tr>
                        </thead>
                        <tbody>
                          {purchaseHistoryRows.map((h) => (
                            <tr key={h.id} className="border-t align-top">
                              <td className="p-2 whitespace-nowrap font-medium">{purchaseTarget.name}</td>
                              <td className="p-2 whitespace-nowrap">{new Date(h.date).toLocaleString()}</td>
                              <td className="p-2 whitespace-nowrap">{h.reference || '-'}</td>
                              <td className="p-2 min-w-[180px]">{h.notes || '-'}</td>
                              <td className="p-2 whitespace-nowrap">{h.variant && h.variant !== NO_VARIANT ? h.variant : 'Default'}</td>
                              <td className="p-2 whitespace-nowrap">{h.color && h.color !== NO_COLOR ? h.color : 'Default'}</td>
                              <td className="p-2 text-right">{toNonNegativeNumber(h.quantity)}</td>
                              <td className="p-2 text-right">₹{toNonNegativeNumber(h.unitPrice).toFixed(2)}</td>
                              <td className="p-2 text-right">{toNonNegativeNumber(h.previousStock)}</td>
                              <td className="p-2 text-right">₹{toNonNegativeNumber(h.previousBuyPrice).toFixed(2)}</td>
                              <td className="p-2 text-right">₹{toNonNegativeNumber(h.nextBuyPrice).toFixed(2)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
