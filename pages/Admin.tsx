...
export default function Admin() {
  ...
  const roundDisplayNumber = (value: any) => Math.ceil(toNonNegativeNumber(value));
  ...
  const saveProduct = async (keepOpenForNext = false) => {
    ...
    try {
      ...
      if (editingProduct) {
        ...
        setProducts(updated);
        if (keepOpenForNext && currentEditingId) {
          const orderedProducts = getFilteredProducts(updated);
          const currentIndex = orderedProducts.findIndex(product => product.id === currentEditingId);
          const nextProduct = currentIndex >= 0 ? orderedProducts[currentIndex + 1] : null;
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="border rounded-lg border-dashed hover:bg-muted/10 transition-colors p-2">
                          <button
                            ...
                            className="h-24 w-24 bg-white rounded-md overflow-hidden border flex items-center justify-center shadow-sm mx-auto text-muted-foreground hover:border-primary hover:text-primary transition-colors"
                          >
                            {formData.image ? (
                          ...
                          <div className="grid grid-cols-7 gap-2 px-2 py-1 border-t" key={getRowKey(row.variant || NO_VARIANT, row.color || NO_COLOR)}>
                            ...
                            <div>
                              <Input type="number" min="0" value={row.stock ?? 0} placeholder={String(getSuggestedStock(row.totalPurchase, row.totalSold))} onChange={e => {
                                ...
                                next[idx] = { ...next[idx], stock: e.target.value === '' ? '' : Math.max(0, Number(sanitizeIntegerInput(e.target.value)) || 0) };
                            ...
                            <Input type="number" min="0" step={1} value={row.buyPrice ?? ''} placeholder="0" onChange={e => {
                              const next = [...(formData.stockByVariantColor || [])];
                              next[idx] = { ...next[idx], buyPrice: e.target.value === '' ? '' : Math.max(0, Number(sanitizeIntegerInput(e.target.value)) || 0) };
                              setFormData({ ...formData, stockByVariantColor: next });
                            }} />
                            <Input type="number" min="0" step={1} value={row.sellPrice ?? ''} placeholder="0" onChange={e => {
                              const next = [...(formData.stockByVariantColor || [])];
                              next[idx] = { ...next[idx], sellPrice: e.target.value === '' ? '' : Math.max(0, Number(sanitizeIntegerInput(e.target.value)) || 0) };
                              setFormData({ ...formData, stockByVariantColor: next });
                            }} />
                            <Input type="number" min="0" step={1} value={row.totalPurchase ?? ''} placeholder="0" onChange={e => {
                              const next = [...(formData.stockByVariantColor || [])];
                              next[idx] = { ...next[idx], totalPurchase: e.target.value === '' ? '' : Math.max(0, Number(sanitizeIntegerInput(e.target.value)) || 0) };
                              setFormData({ ...formData, stockByVariantColor: next });
                            }} />
                            <Input type="number" min="0" step={1} value={row.totalSold ?? ''} placeholder="0" onChange={e => {
                              const next = [...(formData.stockByVariantColor || [])];
                              next[idx] = { ...next[idx], totalSold: e.target.value === '' ? '' : Math.max(0, Number(sanitizeIntegerInput(e.target.value)) || 0) };
                              setFormData({ ...formData, stockByVariantColor: next });
                            }} />
                          </div>
                        ))}
                    ...
                    <div className="grid grid-cols-2 gap-3 justify-items-start">
                      <div className="space-y-2 col-span-2 max-w-[240px] w-full">
                        ...
                        {(!formData.variants?.length && !formData.colors?.length) ? (
                          <Input
                            ...
                            onChange={e => setFormData({ ...formData, stock: sanitizeIntegerInput(e.target.value) })}
                      ...
                      <div className="space-y-2 max-w-[240px] w-full">
                        ...
                        <div className="relative w-full">
                          ...
                          <Input type="number" step={1} className="w-full pl-6" value={formData.buyPrice ?? ''} onChange={e => setFormData({ ...formData, buyPrice: sanitizeIntegerInput(e.target.value) })} placeholder="0" disabled={!!(formData.variants?.length || formData.colors?.length)} />
                        </div>
                      </div>
                      ...
                      <div className="space-y-2 max-w-[240px] w-full">
                        ...
                        <div className="relative w-full">
                          <span className="absolute left-2.5 top-2.5 text-muted-foreground text-xs">₹</span>
                          <Input type="number" step={1} className="w-full pl-6 font-bold text-primary" value={formData.sellPrice ?? ''} onChange={e => setFormData({ ...formData, sellPrice: sanitizeIntegerInput(e.target.value) })} placeholder="0" disabled={!!(formData.variants?.length || formData.colors?.length)} />
                        </div>
                      </div>

                      <div className="space-y-2 max-w-[240px] w-full">
                        <Label>Total Purchase <span className="text-muted-foreground">(Optional)</span></Label>
                        <Input className="w-full" type="number" min="0" step={1} value={formData.totalPurchase ?? ''} onChange={e => setFormData({ ...formData, totalPurchase: sanitizeIntegerInput(e.target.value) })} placeholder="0" />
                      </div>

                      <div className="space-y-2 max-w-[240px] w-full">
                        <Label>Total Sold <span className="text-muted-foreground">(Optional)</span></Label>
                        <Input className="w-full" type="number" min="0" step={1} value={formData.totalSold ?? ''} onChange={e => setFormData({ ...formData, totalSold: sanitizeIntegerInput(e.target.value) })} placeholder="0" />
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
          <Card className="w-full max-w-lg">
            ...
            <CardContent className="space-y-3">
              ...
              <Input type="number" step={1} placeholder="Purchase quantity" value={purchaseQty} onChange={(e) => setPurchaseQty(sanitizeIntegerInput(e.target.value))} />
              <Input type="number" step={1} placeholder="Purchase unit price" value={purchasePrice} onChange={(e) => setPurchasePrice(sanitizeIntegerInput(e.target.value))} />
              <div className="flex gap-2">
                ...
                <Input type="number" step={1} placeholder="New buy price (you can edit)" value={purchaseNextBuyPrice} onChange={(e) => setPurchaseNextBuyPrice(sanitizeIntegerInput(e.target.value))} />
              </div>
