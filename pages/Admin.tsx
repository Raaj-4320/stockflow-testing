...
export default function Admin() {
  ...
  const handleDownloadCategoryPDF = async () => {
    ...
    const renderPageHeader = (categoryName: string, isContinuation: boolean) => {
      doc.setFontSize(18);
      doc.setTextColor(40, 40, 40);
      doc.setFont('helvetica', 'bold');
      ...
      const categoryPrefix = isContinuation ? 'Category (cont.):' : 'Category:';
      doc.text(`${categoryPrefix} ${categoryName}`, margin, 29);
      doc.setDrawColor(200, 200, 200);
      doc.line(margin, headerBottomY, pageWidth - margin, headerBottomY);
    };

    const contentStartY = headerBottomY + 5;
    let y = contentStartY;

    const ensureSpace = (requiredHeight: number, categoryName: string) => {
      if (y + requiredHeight <= bottomSafeY) return;
      doc.addPage();
      renderPageHeader(categoryName, true);
      y = contentStartY;
    };
    ...
    for (let categoryIndex = 0; categoryIndex < sortedCategories.length; categoryIndex += 1) {
      ...
      renderPageHeader(categoryName, false);
      y = contentStartY;

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.setTextColor(107, 114, 128);
      doc.text(`Products in ${categoryName}`, margin, y);
      y += sectionGap;
      ...
      for (const product of productsInCategory) {
        ...
        ensureSpace(cardHeight + rowGap, categoryName);
