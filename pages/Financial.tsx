...
const marginForItem = (item: Transaction['items'][number]) => {
  const qty = safe(item.quantity);
  const sell = safe(item.sellPrice);
  const buy = safe(item.buyPrice);
  return qty * (sell - buy);
};

const cogsForItem = (item: Transaction['items'][number]) => safe(item.quantity) * safe(item.buyPrice);
...
export default function Financial() {
  ...
  const model = useMemo(() => {
    ...
    const cogs = sales.reduce((s, t) => s + t.items.reduce((ss, i) => ss + cogsForItem(i), 0), 0);
    const grossProfitFromItems = sales.reduce((s, t) => s + t.items.reduce((ss, i) => ss + marginForItem(i), 0), 0);
    const returnsValue = returns.reduce((s, t) => s + Math.abs(safe(t.total)), 0);
    const returnsMarginLoss = returns.reduce((s, t) => s + Math.abs(t.items.reduce((ss, i) => ss + marginForItem(i), 0)), 0);
    ...
    const productSales = groupBy(
      sales.flatMap(t => t.items.map(i => ({ key: i.id, name: i.name, qty: safe(i.quantity), margin: marginForItem(i) }))),
      r => r.key,
    );
    ...
    const revenueProfitTrend = dayKeys.map((d) => {
      ...
      const dayProfit = daySales.reduce((s, t) => s + t.items.reduce((ss, i) => ss + marginForItem(i), 0), 0) - dayReturns.reduce((s, t) => s + Math.abs(t.items.reduce((ss, i) => ss + marginForItem(i), 0)), 0);
