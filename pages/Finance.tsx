...
export default function Finance() {
  ...
  const selectedLayerIntelligenceSummary = useMemo(() => ({
    corrections: filteredCashbookRows.filter(row => row.layerType === 'adjustment').length,
    financialCorrections: filteredCashbookRows.filter(row => row.correctionImpactClass === 'financial').length,
    metadataOnlyCorrections: filteredCashbookRows.filter(row => row.correctionImpactClass === 'metadata_only').length,
    needsReview: filteredCashbookRows.filter(row => row.riskLevel === 'medium' || row.riskLevel === 'high').length,
  ...
  const cashbookExportSummaryRows = useMemo(() => ([
    ...
    { Metric: 'Export Summary — Filtered Dataset', Value: 'before reporting-layer + audit-layer selection' },
    { Metric: 'Rows in filtered dataset', Value: String(baseFilteredCashbookRows.length) },
    { Metric: 'Rows needing review (filtered dataset)', Value: String(cashbookIntelligenceSummary.needsReview) },
    { Metric: 'Adjustment rows (filtered dataset)', Value: String(cashbookIntelligenceSummary.corrections) },
    { Metric: 'Financial adjustment rows (filtered dataset)', Value: String(cashbookIntelligenceSummary.financialCorrections) },
    { Metric: 'Metadata-only adjustment rows (filtered dataset)', Value: String(cashbookIntelligenceSummary.metadataOnlyCorrections) },
    { Metric: 'Store-credit return rows (filtered dataset)', Value: String(cashbookIntelligenceSummary.returnsWithStoreCredit) },
    { Metric: 'Large balance movement rows (filtered dataset)', Value: String(cashbookIntelligenceSummary.largeBalanceMovements) },
    { Metric: 'Mixed refund events (filtered dataset)', Value: String(cashbookIntelligenceSummary.mixedRefundEvents) },
    { Metric: 'Export Summary — Selected Reporting Layer', Value: 'after reporting-layer + audit-layer selection' },
    { Metric: 'Reporting layer', Value: reportingLayerMode },
    { Metric: 'Rows exported in selected layer', Value: String(filteredCashbookRows.length) },
    { Metric: 'Rows needing review (selected layer)', Value: String(selectedLayerIntelligenceSummary.needsReview) },
    { Metric: 'Adjustment rows (selected layer)', Value: String(selectedLayerIntelligenceSummary.corrections) },
    { Metric: 'Financial adjustment rows (selected layer)', Value: String(selectedLayerIntelligenceSummary.financialCorrections) },
    { Metric: 'Metadata-only adjustment rows (selected layer)', Value: String(selectedLayerIntelligenceSummary.metadataOnlyCorrections) },
    { Metric: 'Gross Sales (selected layer)', Value: cashbookRollups.grossSales.toFixed(2) },
    { Metric: 'Returns (selected layer)', Value: cashbookRollups.salesReturns.toFixed(2) },
    { Metric: 'Credit Due Created (selected layer)', Value: cashbookRollups.creditDueCreated.toFixed(2) },
    ...
    { Metric: 'Cash In (selected layer)', Value: cashbookRollups.cashIn.toFixed(2) },
    { Metric: 'Cash Out (selected layer)', Value: cashbookRollups.cashOut.toFixed(2) },
    { Metric: 'Net Cash Movement (selected layer)', Value: (cashbookRollups.cashIn - cashbookRollups.cashOut).toFixed(2) },
    { Metric: 'Gross Profit (selected layer)', Value: cashbookRollups.grossProfit.toFixed(2) },
    { Metric: 'Net Profit (selected layer)', Value: cashbookRollups.netProfit.toFixed(2) },
    { Metric: 'Operational Sales (selected layer)', Value: realActivityRollups.realSales.toFixed(2) },
    { Metric: 'Operational Returns (selected layer)', Value: realActivityRollups.realReturns.toFixed(2) },
    { Metric: 'Net adjustment impact (selected layer)', Value: realActivityRollups.netAdjustmentImpact.toFixed(2) },
    { Metric: 'Net financial adjustment impact (selected layer)', Value: realActivityRollups.netFinancialAdjustmentImpact.toFixed(2) },
  ]), [cashbookScope, cashbookFromDate, cashbookToDate, cashbookTypeFilter, cashbookAuditFilter, reportingLayerMode, cashbookCustomerQuery, baseFilteredCashbookRows.length, filteredCashbookRows.length, cashbookRollups, dueStoreCreditSummary.totalDue, dueStoreCreditSummary.totalStoreCredit, cashbookIntelligenceSummary, selectedLayerIntelligenceSummary, realActivityRollups]);
