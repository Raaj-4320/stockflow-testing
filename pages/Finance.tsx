...
export default function Finance() {
  ...
  const [openHistoryRows, setOpenHistoryRows] = useState<Record<string, boolean>>({});
  ...
  }, [filteredCashHistory, data.transactions]);
  ...
  const expenseActivities: ExpenseActivity[] = useMemo(() => data.expenseActivities || [], [data]);
  ...
  const addExpense = async () => {
    ...
    await persistState({
      ...
      expenseActivities: appendExpenseActivity(data.expenseActivities || [], 'add_expense', `Added ${expense.title} (${formatINR(expense.amount)}) in ${expense.category}`)
    });
  ...
  const removeExpense = async (id: string) => {
    ...
    await persistState({
      ...
      expenseActivities: item
        ? appendExpenseActivity(data.expenseActivities || [], 'delete_expense', `Deleted ${item.title} (${formatINR(item.amount)})`)
  ...
  const addExpenseCategory = async () => {
    ...
    await persistState({ ...data, expenseCategories: categories, expenseActivities: appendExpenseActivity(data.expenseActivities || [], 'add_category', `Added category ${name}`) });
    setNewCategory('');
  };
            ...
            <Card className="border-slate-200 shadow-sm bg-white">
              ...
              <CardContent className="space-y-3 pt-5">
                {filteredCashHistory.map(session => {
                  ...
                  const systemCashTotal = session.systemCashTotal ?? computedTotals.systemCashTotal;
                  const difference = session.difference ?? ((session.closingBalance ?? 0) - (session.openingBalance + systemCashTotal));
                  const isOpen = !!openHistoryRows[session.id];
                      ...
                      <div className="flex flex-col gap-2 px-4 py-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0">
                          ...
                          <div className="mt-2">
                            ...
                            <div className="mt-0.5 text-xs text-slate-600">Difference = Counted cash − System cash</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Button type="button" variant="outline" size="sm" onClick={() => setOpenHistoryRows(prev => ({ ...prev, [session.id]: !prev[session.id] }))}>{isOpen ? 'Hide details' : 'View details'}</Button>
                        </div>
                      </div>
                      ...
                      {isOpen && (
                        <div className="border-t border-slate-200 px-4 py-4">
                          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                            <div className="rounded-lg border border-slate-200 bg-white p-3"><div className="text-[11px] font-medium text-slate-500">Difference (Counted − System)</div><div className={`mt-1 text-sm font-semibold ${difference < 0 ? 'text-rose-700' : difference === 0 ? 'text-emerald-700' : 'text-slate-900'}`}>{formatINR(difference)}</div></div>
                            <div className="rounded-lg border border-slate-200 bg-white p-3"><div className="text-[11px] font-medium text-slate-500">Status</div><div className="mt-1 text-sm font-semibold text-slate-900">{session.status === 'closed' ? 'Closed' : 'Open'}</div></div>
                          </div>
                          <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600"><span className="font-medium text-slate-700">Tip:</span> If “Short”, collect missing cash. If “Over”, verify transactions or cash count.</div>
                        </div>
                      )}
                    </div>
                  );
