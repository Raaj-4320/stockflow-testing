...
import { loadData, saveData, processTransaction, getSaleSettlementBreakdown } from '../services/storage';
...
const getSessionCashTotals = (transactions: Transaction[], expenses: Expense[], sessionStartIso: string, sessionEndIso?: string, sessionId?: string) => {
  ...
  const cashSales = scopedTransactions
    ...
    .reduce((sum, t) => sum + getSaleSettlementBreakdown(t).cashPaid, 0);
  const cashRefunds = scopedTransactions
    .filter(t => t.type === 'return' && t.paymentMethod === 'Cash')
  ...
  financeShiftDiag('[FIN][SESSION][TX_FILTER]', {
    ...
    totalTransactionsFound: scopedTransactions.length,
    totalExpensesFound: windowExpenses.length,
    sampleTxIds: scopedTransactions.slice(0, 5).map(tx => tx.id),
  });
...
export default function Finance() {
  ...
  const todayFinanceBreakdown = useMemo(() => {
    ...
    const cashSales = sales.reduce((s, t) => s + getSaleSettlementBreakdown(t).cashPaid, 0);
    const cashCollections = scopedTransactions
      .filter(t => t.type === 'payment' && t.paymentMethod === 'Cash')
      .reduce((s, t) => s + Math.abs(t.total), 0);
    const creditSales = sales.reduce((s, t) => s + getSaleSettlementBreakdown(t).creditDue, 0);
    const onlineSales = sales.reduce((s, t) => s + getSaleSettlementBreakdown(t).onlinePaid, 0);
