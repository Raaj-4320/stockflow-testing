...
export default function Finance() {
  ...
  const cashbookRows = useMemo(() => {
    ...
    events.forEach((event) => {
      ...
      if (event.kind === 'updated' && event.updated) {
        ...
        const effectSummary = updatedType === 'return'
        ...
        rows.push({
          ...
          notes: `Original Tx: ${updatedEvent.originalTransactionId} • Updated Tx: ${updatedEvent.updatedTransactionId}`,
    ...
    return rows.map(normalizeCashbookRowMoney);
  ...
  const filteredCashbookRows = useMemo(() => {
    if (cashbookAuditFilter === 'needs_review') {
      return baseFilteredCashbookRows.filter(row => row.riskLevel === 'medium' || row.riskLevel === 'high');
    }
    if (cashbookAuditFilter === 'corrections_only') {
      return baseFilteredCashbookRows.filter(row => isCorrectionRowType(row.type));
    }
    return baseFilteredCashbookRows;
  }, [baseFilteredCashbookRows, cashbookAuditFilter]);
  ...
  ]), [cashbookFromDate, cashbookToDate, cashbookTypeFilter, cashbookAuditFilter, cashbookCustomerQuery, filteredCashbookRows.length, cashbookRollups, dueStoreCreditSummary.totalDue, dueStoreCreditSummary.totalStoreCredit, cashbookIntelligenceSummary]);
                  ...
                  <table className="min-w-[1900px] w-full text-xs">
                    ...
                    <tbody>
                      ...
                      {!filteredCashbookRows.length && (
                        <tr>
                          <td className="p-4 text-center text-muted-foreground" colSpan={23}>No cashbook rows for selected filters.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
