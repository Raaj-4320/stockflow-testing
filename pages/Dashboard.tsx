...
export default function Dashboard() {
  ...
  const allCustomerDashboardRows = useMemo(() => customers.map((customer) => {
    const breakdown = getCustomerCompositeReceivableBreakdown(customer.id, customers, transactions, upfrontOrders);
    return { ...customer, receivable: Math.max(0, breakdown.totalDue), storeCredit: Math.max(0, breakdown.storeCredit || 0) } as CustomerReceivableRow;
  ...
  const allPartyDashboardRows = useMemo<PartyPayableRow[]>(() => {
    ...
    partyMap.forEach((party, id) => {
      ...
      partyMap.set(id, { ...party, payable, dueOrders: partyDueOrders, partyCredit: 0 });
    });
    return Array.from(partyMap.values()).sort((a, b) => a.name.localeCompare(b.name));
  }, [parties, orders, supplierPayments]);
  const payablePartyRows = useMemo(() => allPartyDashboardRows.filter((p) => p.payable > 0), [allPartyDashboardRows]);
  const creditPartyRows = useMemo(() => allPartyDashboardRows.filter((p) => Math.max(0, Number(p.partyCredit || 0)) > 0), [allPartyDashboardRows]);
  ...
  const customerStatement = useMemo(() => {
    ...
    const upfrontEffects = buildUpfrontOrderLedgerEffects(upfrontOrders.filter((o) => o.customerId === selectedCustomer.id), [selectedCustomer]).filter((effect) => effect.type !== 'legacy_custom_order_info');
    const events = [
    ...
    events.forEach((event) => {
      ...
      if (txKind === 'sale') {
        ...
        const dueReduced = (explicitApplied > 0 && explicitApplied <= runningBalance)
          ? Math.min(amount, explicitApplied, runningBalance)
          : Math.min(runningBalance, amount);
    ...
    const balanceDue = Math.max(0, Number(compositeByCustomer.get(selectedCustomer.id) || 0));
    const displayRows = [...rows].reverse();
    return { rows, displayRows, totalCreditSales, totalPayments, totalStoreCreditUsed, totalStoreCreditAdded, balanceDue };
  }, [selectedCustomer, transactions, canonicalSnapshot, upfrontOrders, compositeByCustomer]);
              ...
              <div key={p.id} className="flex items-center justify-between border rounded-lg p-3 gap-3">
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className="mt-2 flex gap-2 justify-end">
                    ...
                    {supplierDashboardTab === 'payable' && <Button size="sm" variant="outline" onClick={() => openPayModal(p)}>Pay</Button>}
                  </div>
                </div>
              </div>
            ))}
            {supplierDashboardTab === 'payable' && !payablePartyRows.length && <p className="text-sm text-muted-foreground">No payable parties.</p>}
            {supplierDashboardTab === 'credit' && !creditPartyRows.length && <p className="text-sm text-muted-foreground">No party credits recorded yet. Supplier credit will appear here after the supplier-credit model is implemented.</p>}
