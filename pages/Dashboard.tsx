...
import { buildUpfrontOrderLedgerEffects, createSupplierPayment, deleteLegacySupplierPaymentGroup, deleteSupplierPayment, deleteTransaction, getCanonicalCustomerBalanceSnapshot, getCanonicalReturnAllocation, getCustomerPaymentAllocationForDisplayedReceivable, getPurchaseOrders, getPurchaseParties, getHistoricalAwareSaleSettlement, getSaleSettlementBreakdown, loadData, processTransaction, updateSupplierPayment, updateTransaction } from '../services/storage';
...
export default function Dashboard() {
  ...
  const customerStatement = useMemo(() => {
    ...
    const customDue = Math.max(0, Number(customReceivableByCustomer.get(selectedCustomer.id) || 0));
    const canonicalDue = Math.max(0, Number(canonicalSnapshot.balances.get(selectedCustomer.id)?.totalDue || 0));
    const balanceDue = Math.max(0, canonicalDue + customDue);
    const displayRows = [...rows].reverse();
    return { rows, displayRows, totalCreditSales, totalPayments, totalStoreCreditUsed, totalStoreCreditAdded, balanceDue };
  }, [selectedCustomer, transactions, canonicalSnapshot, upfrontOrders, customReceivableByCustomer]);
  ...
  const handleReceive = () => {
    ...
    const allocation = getCustomerPaymentAllocationForDisplayedReceivable({ paymentAmount: amount, displayedReceivable: receiveCurrentDue });
    (tx as any).paymentAppliedToReceivable = allocation.paymentAppliedToReceivable;
    (tx as any).storeCreditCreated = allocation.storeCreditCreated;
