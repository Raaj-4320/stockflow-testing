  ...
  const partyStatement = useMemo(() => {
    ...
    partyOrders.forEach(order => {
      ...
      purchaseEvents.push({
        ...
        description: `PO ${order.billNumber || order.id.slice(-6)} • ${getPurchaseOrderProductSummary(order)}${order.status ? ` • ${order.status}` : ''}${sameOrderPartyCreditUsed > 0 ? ` • Party Credit Used ${formatINRPrecise(sameOrderPartyCreditUsed)}` : ''}`,
        debit: orderTotal,
