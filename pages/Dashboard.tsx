...
const customerReceivables = useMemo<CustomerReceivableRow[]>(() => customers
  ...
  const allPartyDashboardRows = useMemo<PartyPayableRow[]>(() => {
    ...
    partyMap.forEach((party, id) => {
      ...
      const fallbackCredit = activeSupplierEntries.reduce((sum, payment) => {
        const paymentCredit = Math.max(0, Number(payment.partyCreditCreated || 0));
        if (paymentCredit <= 0) return sum;
        const partyMatch = payment.partyId ? payment.partyId === id : (String(payment.partyName || '').trim().toLowerCase() === String(party.name || '').trim().toLowerCase());
        if (!partyMatch) return sum;
        ...
        if ((import.meta as any).env?.DEV) {
          console.info('[PARTY_CREDIT_RECON]', {
            ...
            partyCreditCreated: paymentCredit,
            reason: 'active supplier payment credit missing from ledger; using dashboard fallback',
          });
        }
        return sum + paymentCredit;
      }, 0);
