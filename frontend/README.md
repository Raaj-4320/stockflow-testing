...
## Shell structure
- `app/`: App Router routes (placeholder pages only).
- `shared/components/`: reusable shell components (`AppShell`, `PageHeader`).
- `features/`: future feature modules (currently placeholder).
- `lib/`, `services/`: reserved for future utilities/adapters.


## Typed client skeleton (Phase 4)
- `lib/env.ts`, `lib/errors.ts`, `services/apiClient.ts`, and `services/procurementClient.ts` are present.
- These clients are **not wired to UI routes yet**.
